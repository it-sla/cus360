# Graph Report - customer360  (2026-09-17)

## Corpus Check
- 1366 files · ~635,956 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 7507 nodes · 10515 edges · 1386 communities (1285 shown, 101 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 402 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `76479358`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- chart-context.tsx
- time-series-chart-shell.tsx
- CrmSessionManager
- gray
- gray
- search
- search
- .agents/skills/design-system/scripts/slide_search_core.py
- test_crm_reliability.py
- .opencode/skills/design-system/scripts/slide_search_core.py
- devDependencies
- react
- spacing
- TestTailwindConfigGenerator
- .agents/skills/design-system/scripts/html-token-validator.py
- .opencode/skills/design-system/scripts/html-token-validator.py
- TestTailwindConfigGenerator
- api
- crm_worker.py
- compilerOptions
- BM25
- crm_backfills.py
- Tailwind CSS Utility Reference
- BM25
- models.py
- spacing
- DesignSystemGenerator
- RuntimeError
- components.json
- Customer360.tsx
- .agents/skills/design-system/scripts/generate-slide.py
- api.ts
- compilerOptions
- .opencode/skills/design-system/scripts/generate-slide.py
- DesignSystemGenerator
- color
- color
- .agents/skills/design-system/scripts/fetch-background.py
- TailwindConfigGenerator
- .opencode/skills/design-system/scripts/fetch-background.py
- TailwindConfigGenerator
- BM25
- BusinessAnalytics.tsx
- BM25
- .opencode/skills/ui-ux-pro-max/scripts/design_system.py
- .agents/skills/design/scripts/icon/generate.py
- fontSize
- ShadcnInstaller
- main
- utils.py
- .opencode/skills/design/scripts/icon/generate.py
- fontSize
- ShadcnInstaller
- main
- TestShadcnInstaller
- radius
- TestShadcnInstaller
- .agents/skills/ui-ux-pro-max/scripts/design_system.py
- .agents/skills/brand/scripts/extract-colors.cjs
- .agents/skills/brand/scripts/validate-asset.cjs
- pie-context.tsx
- chart-tooltip.tsx
- .opencode/skills/brand/scripts/extract-colors.cjs
- .opencode/skills/brand/scripts/validate-asset.cjs
- test_pipeline_sync.py
- test_crm_parser.py
- .agents/skills/design-system/templates/design-tokens-starter.json
- .opencode/skills/design-system/templates/design-tokens-starter.json
- .agents/skills/design-system/scripts/validate-tokens.cjs
- card
- .generate_config_string
- .opencode/skills/design-system/scripts/validate-tokens.cjs
- card
- .generate_config_string
- .agents/skills/brand/scripts/inject-brand-context.cjs
- .agents/skills/design-system/scripts/embed-tokens.cjs
- patch
- .agents/skills/ui-styling/scripts/tests/test_tailwind_config_gen.py
- search
- .opencode/skills/brand/scripts/inject-brand-context.cjs
- .opencode/skills/design-system/scripts/embed-tokens.cjs
- duration
- patch
- .opencode/skills/ui-styling/scripts/tests/test_tailwind_config_gen.py
- search
- .agents/skills/design/scripts/logo/generate.py
- .agents/skills/design-system/scripts/generate-tokens.cjs
- use-chart-interaction.ts
- duration
- ._base_config
- Changes shipped this session
- dropdown-menu.tsx
- .opencode/skills/design/scripts/logo/generate.py
- .opencode/skills/design-system/scripts/generate-tokens.cjs
- button
- ._base_config
- .agents/skills/brand/scripts/sync-brand-to-tokens.cjs
- _run
- dependencies
- UniversalSearch.tsx
- .agents/skills/ui-ux-pro-max/scripts/core.py
- sheet.tsx
- .agents/skills/ui-ux-pro-max/scripts/tests/test_core.py
- .opencode/skills/brand/scripts/sync-brand-to-tokens.cjs
- _run
- remote-compose.sh
- button
- navigation-menu.tsx
- select.tsx
- input
- radius
- detect_domain
- animation.ts
- main.py
- MasterAirWaybills.tsx
- detect_domain
- Tailwind CSS Utility Reference
- sm
- Brand Guidelines v1.0
- area.tsx
- email_notifications.py
- Brand Guidelines v1.0
- Design
- radius
- lg
- Canvas Design System
- xl
- User
- series-dash-tail-overlay.tsx
- .agents/skills/ui-ux-pro-max/scripts/validate_data.py
- key_insights
- normalize_icris
- y-domain-utils.ts
- xl
- none
- .opencode/skills/ui-ux-pro-max/scripts/validate_data.py
- devDependencies
- .agents/skills/brand/scripts/tests/test_sync_brand_to_tokens.py
- main
- UUID
- Design
- Canvas Design System
- auth.py
- Prerequisites
- Prerequisites
- Form & Input Components
- Tailwind CSS Responsive Design
- x-axis.tsx
- Form & Input Components
- .__init__
- .temp_project
- .test_get_installed_components_empty
- Shipment
- tsconfig.json
- .opencode/skills/brand/scripts/tests/test_sync_brand_to_tokens.py
- main
- DataQualityIssue
- Tailwind CSS Responsive Design
- Typography Specifications
- Typography Specifications
- ring
- Logo Usage Rules
- .__init__
- .temp_project
- .agents/skills/ui-styling/scripts/tests/test_shadcn_add.py
- .test_add_components_no_config
- Component Specifications
- shadcn/ui Accessibility Patterns
- .test_init_dry_run
- .test_check_shadcn_config_exists
- .test_check_shadcn_config_not_exists
- .test_add_components_no_components
- Logo Usage Rules
- Component Specifications
- shadcn/ui Accessibility Patterns
- .test_init_default_typescript
- .test_generate_javascript_config
- .test_write_config
- Asset Approval Checklist
- Logo AI Prompt Engineering
- .test_init_framework
- .test_custom_output_path
- Asset Approval Checklist
- Logo AI Prompt Engineering
- Color Palette Management
- cmdk
- date-fns
- 2. Parser (`crm_parser.py`)
- echarts-for-react
- CIP Deliverable Guide
- App.tsx
- States and Variants
- maplibre-gl
- motion
- @radix-ui/react-avatar
- @radix-ui/react-checkbox
- @radix-ui/react-dialog
- @radix-ui/react-dropdown-menu
- @radix-ui/react-hover-card
- DataQuality.tsx
- @radix-ui/react-popover
- UI Styling Skill
- Color Palette Management
- @radix-ui/react-separator
- @radix-ui/react-slot
- Major
- input
- test_customer_segments.py
- API Reference
- grid.tsx
- react-is
- Data Rules
- tailwind-merge
- CIP Deliverable Guide
- zod
- .opencode/skills/ui-styling/scripts/tests/test_shadcn_add.py
- .test_add_components_no_config
- .test_add_all_components_dry_run
- .test_list_installed_with_components
- .test_init_custom_project_root
- .test_init_dry_run
- .test_check_shadcn_config_exists
- .test_check_shadcn_config_not_exists
- .test_get_installed_components_empty
- .test_get_installed_components_no_config
- .test_add_components_no_components
- .test_generate_typescript_config
- .test_init_default_typescript
- .test_generate_javascript_config
- .test_generate_config_with_colors
- .test_write_config
- .test_write_config_creates_content
- .test_full_configuration_typescript
- .test_init_framework
- .test_custom_output_path
- .test_default_content_paths_react
- .test_add_colors
- backup.sh
- export_standalone.sh
- restore.sh script
- States and Variants
- UI Styling Skill
- Workflow
- Workflow
- Design System
- Tailwind CSS Customization
- Design System
- Tailwind CSS Customization
- Routing by Task Type
- shadcn/ui Theming & Customization
- Routing by Task Type
- shadcn/ui Theming & Customization
- Asset Organization Guide
- Primary Color Meanings
- Core Logo Types
- Customer 360 - AI Architecture & Domain Context
- Asset Organization Guide
- Primary Color Meanings
- Core Logo Types
- Brand Consistency Checklist
- CIP Mockup Prompt Engineering
- Color Semantics
- Brand Consistency Checklist
- CIP Mockup Prompt Engineering
- Color Semantics
- Design System Master File
- Design System Master File
- Design Principles
- Design Principles
- Design Principles
- Design Principles
- CIP Design Reference
- Icon Design Reference
- Copywriting Formulas
- Copywriting Formulas
- CIP Design Reference
- Icon Design Reference
- Copywriting Formulas
- tasteskill: Anti-Slop Frontend Skill
- Appendix B - Canonical Sources (read these before reinventing)
- Copywriting Formulas
- Banner Design - Multi-Format Creative Banner System
- Messaging Framework
- Brand Voice Framework
- Layout Patterns
- Tailwind Integration
- Layout Patterns
- Banner Design - Multi-Format Creative Banner System
- Messaging Framework
- Brand Voice Framework
- Layout Patterns
- Tailwind Integration
- Layout Patterns
- .agents/skills/brand/references/update.md
- Logo Design Reference
- Token Architecture
- .opencode/skills/brand/references/update.md
- Logo Design Reference
- Token Architecture
- Primitive Tokens
- Primitive Tokens
- 4. DESIGN ENGINEERING DIRECTIVES (Bias Correction)
- Core Visual Elements
- CIP Design Style Guide
- Core Visual Elements
- CIP Design Style Guide
- Customer 360
- Customer 360 domain rules
- Brand
- Slide Strategies
- Component Tokens
- Slide Strategies
- UploadFile
- frontend/package.json
- Brand
- Slide Strategies
- Component Tokens
- 10. REFERENCE VOCABULARY (Pattern Names the Agent Should Know)
- Slide Strategies
- plugins
- 9. AI TELLS (Forbidden Patterns)
- Slides Reference
- HTML Slide Template
- HTML Slide Template
- Customer 360 Backend API Reference
- Open Questions
- Slides Reference
- HTML Slide Template
- APPENDICES - Real Source-Backed Reference Material
- 11. REDESIGN PROTOCOL
- 3. DEFAULT ARCHITECTURE & CONVENTIONS
- 6. PERFORMANCE & ACCESSIBILITY GUARDRAILS
- HTML Slide Template
- Slides
- CLAUDE.md
- Slides
- Brand Guidelines Template
- Rankings.tsx
- sidebar-component.tsx
- Customer 360 System Memory Bank & Architecture Specification
- Brand Guidelines Template
- $type
- 12. THE BLOCK LIBRARY (Contract - Implementations Land Here Iteratively)
- 5. CONTEXT-AWARE PROACTIVITY
- 8. DARK MODE PROTOCOL
- Courier Intelligence Dashboard Design Reference
- value_error
- React + TypeScript + Vite
- test_ae_targets.py
- AEPerformance.tsx
- projection-utils.ts
- 7. DIAL DEFINITIONS (Technical Reference)
- Role-Based Access Control — Plan & Status
- UX System Flow — Customer 360
- Development
- 3. Feature work this session (chronological)
- 2. Findings by area
- Session Handover — Customer 360
- Core entities
- Leaderboard.tsx
- app-shell.tsx
- dialog.tsx
- $type
- radius
- 8
- destructive-foreground
- muted
- primary-foreground
- secondary-foreground
- .agents/skills/design/references/slides-create.md
- .agents/skills/slides/references/create.md
- .test_add_all_components_dry_run
- .test_list_installed_with_components
- .test_init_custom_project_root
- .test_get_installed_components_no_config
- .test_generate_typescript_config
- .test_generate_config_with_colors
- .test_write_config_creates_content
- .test_full_configuration_typescript
- .test_default_content_paths_react
- .test_add_colors
- Session Handover — Customer 360
- .agents/skills/design/scripts/cip/generate.py
- .opencode/skills/design/references/slides-create.md
- .opencode/skills/slides/references/create.md
- 1. Company-master import — LIVE
- AnalyticsFilterBar.tsx
- Session Handoff Pt.2 — 2026-08-10
- Session Handover — Customer 360
- Session Handover — Customer 360
- 1. Local Docker Workflow (Windows/Mac)
- loading-sweep.tsx
- Alerts.tsx
- admin_bulk_create_users
- test_document_type_analytics.py
- Frontend
- Database
- AeAssignment.tsx
- auth.tsx
- Session Handoff — 2026-08-07 to 2026-08-10
- BM25
- Architecture
- Customer Segmentation Rule
- BLOCKERS
- test_mawbs_pnl_sort.py
- lg
- leaderboard-rankings.tsx
- Profitability.tsx
- sm
- AeTargets.tsx
- DocumentTypeAnalytics.tsx
- Customer 360 — Documentation
- AlertBanner.tsx
- OperationalAnalytics.tsx
- f4a7c9e2b810_add_normalized_company_name_to_call_logs.py
- get_alerts
- Pipeline.tsx
- padding-y
- none
- padding-y
- @base-ui/react
- @carbon/icons-react
- class-variance-authority
- d3-array
- d3-shape
- framer-motion
- html2canvas
- jspdf
- lucide-react
- @number-flow/react
- @radix-ui/react-collapsible
- @radix-ui/react-scroll-area
- @radix-ui/react-select
- react-map-gl
- @tanstack/react-query
- @tanstack/react-table
- @visx/curve
- @visx/event
- @visx/gradient
- @visx/group
- @visx/responsive
- @visx/scale
- @visx/shape
- 1. THE THREE DIALS (Core Configuration)
- test_data_rules.py
- 16
- 1
- 3
- 8
- destructive
- destructive-foreground
- muted
- primary-foreground
- ring
- secondary-foreground
- 12
- 4
- 6
- destructive

## God Nodes (most connected - your core abstractions)
1. `User` - 109 edges
2. `react` - 109 edges
3. `CrmSessionManager` - 61 edges
4. `TailwindConfigGenerator` - 60 edges
5. `commit()` - 57 edges
6. `TailwindConfigGenerator` - 56 edges
7. `one()` - 54 edges
8. `now()` - 54 edges
9. `Shipment` - 54 edges
10. `CrmParseError` - 48 edges

## Surprising Connections (you probably didn't know these)
- `TestDomainDetection` --uses--> `BM25`  [INFERRED]
  .opencode/skills/ui-ux-pro-max/scripts/tests/test_core.py → .agents/skills/design/scripts/cip/core.py
- `TestPersistence` --uses--> `BM25`  [INFERRED]
  .opencode/skills/ui-ux-pro-max/scripts/tests/test_core.py → .agents/skills/design/scripts/cip/core.py
- `TestReasoningMatch` --uses--> `BM25`  [INFERRED]
  .opencode/skills/ui-ux-pro-max/scripts/tests/test_core.py → .agents/skills/design/scripts/cip/core.py
- `TestSearchDomains` --uses--> `BM25`  [INFERRED]
  .opencode/skills/ui-ux-pro-max/scripts/tests/test_core.py → .agents/skills/design/scripts/cip/core.py
- `TestTokenizer` --uses--> `BM25`  [INFERRED]
  .opencode/skills/ui-ux-pro-max/scripts/tests/test_core.py → .agents/skills/design/scripts/cip/core.py

## Import Cycles
- None detected.

## Communities (1386 total, 101 thin omitted)

### Community 0 - "chart-context.tsx"
Cohesion: 0.09
Nodes (36): AreaChart(), AreaChartProps, ChartInner(), ChartInnerProps, DEFAULT_MARGIN, extractAreaConfigs(), AreaChartLoading(), AreaChartLoadingProps (+28 more)

### Community 1 - "time-series-chart-shell.tsx"
Cohesion: 0.06
Nodes (43): CHART_CLIP_PASSTHROUGH, CLIP_EXCLUDED_COMPONENT_NAMES, isChartClipPassthrough(), isClipExcludedComponent(), isPostOverlayComponent(), isUnderlayComponent(), resolveChartChildElement(), UNDERLAY_COMPONENT_NAMES (+35 more)

### Community 2 - "CrmSessionManager"
Cohesion: 0.06
Nodes (102): AeWorkbookError, ValueError, AeTargetWorkbookError, ValueError, LoginRequest, BaseModel, CompanyWorkbookError, ValueError (+94 more)

### Community 3 - "gray"
Cohesion: 0.05
Nodes (53): $type, $value, $type, $value, $type, $value, $type, $value (+45 more)

### Community 4 - "gray"
Cohesion: 0.05
Nodes (53): $type, $value, $type, $value, $type, $value, $type, $value (+45 more)

### Community 5 - "search"
Cohesion: 0.12
Nodes (24): detect_domain(), get_cip_brief(), _load_csv(), Load CSV and return list of dicts, Core search function using BM25, Auto-detect the most relevant domain from query, Main search function with auto-domain detection, Search across all domains and combine results (+16 more)

### Community 6 - "search"
Cohesion: 0.07
Nodes (42): BM25, detect_domain(), get_cip_brief(), _load_csv(), Load CSV and return list of dicts, Core search function using BM25, Auto-detect the most relevant domain from query, Main search function with auto-domain detection (+34 more)

### Community 7 - ".agents/skills/design-system/scripts/slide_search_core.py"
Cohesion: 0.09
Nodes (36): format_context(), format_result(), main(), Format a single search result for display, Format contextual recommendations for display., BM25, calculate_pattern_break(), detect_domain() (+28 more)

### Community 8 - "test_crm_reliability.py"
Cohesion: 0.08
Nodes (46): RetryableConnectorError, _checksum(), EmptyManifestError, parse_manifest_detail(), FastCrmSession, date, run_fast_sync(), CrmSyncItem (+38 more)

### Community 9 - ".opencode/skills/design-system/scripts/slide_search_core.py"
Cohesion: 0.09
Nodes (36): format_context(), format_result(), main(), Format a single search result for display, Format contextual recommendations for display., BM25, calculate_pattern_break(), detect_domain() (+28 more)

### Community 10 - "devDependencies"
Cohesion: 0.09
Nodes (23): autoprefixer, devDependencies, autoprefixer, oxlint, postcss, tailwindcss, @tailwindcss/postcss, @types/node (+15 more)

### Community 11 - "react"
Cohesion: 0.03
Nodes (43): HighlightSegment(), HighlightSegmentProps, SeriesHighlightLayerProps, ExecutiveLayoutProps, KpiCardProps, LucideIcon, Avatar, AvatarFallback (+35 more)

### Community 12 - "spacing"
Cohesion: 0.09
Nodes (22): $type, $value, $type, $value, $type, $value, $type, $value (+14 more)

### Community 13 - "TestTailwindConfigGenerator"
Cohesion: 0.07
Nodes (15): Test adding colors multiple times., Test adding full color palette., Test adding custom breakpoints., Test TailwindConfigGenerator class., Test that adding same plugin twice doesn't duplicate., Test plugin recommendations for Next.js., Test generating config with plugins., Test validating valid configuration. (+7 more)

### Community 14 - ".agents/skills/design-system/scripts/html-token-validator.py"
Cohesion: 0.14
Nodes (24): get_context(), is_allowed_exception(), is_allowed_rgba(), is_inside_block(), load_css_variables(), main(), print_result(), print_summary() (+16 more)

### Community 15 - ".opencode/skills/design-system/scripts/html-token-validator.py"
Cohesion: 0.14
Nodes (24): get_context(), is_allowed_exception(), is_allowed_rgba(), is_inside_block(), load_css_variables(), main(), print_result(), print_summary() (+16 more)

### Community 16 - "TestTailwindConfigGenerator"
Cohesion: 0.07
Nodes (15): Test adding colors multiple times., Test adding full color palette., Test adding custom breakpoints., Test TailwindConfigGenerator class., Test that adding same plugin twice doesn't duplicate., Test plugin recommendations for Next.js., Test generating config with plugins., Test validating valid configuration. (+7 more)

### Community 17 - "api"
Cohesion: 0.17
Nodes (9): api, Shipment, CrmSync(), fmtDate(), CustomerDirectory(), fmt$(), fmtWeight(), STATUS_BADGES (+1 more)

### Community 18 - "crm_worker.py"
Cohesion: 0.16
Nodes (36): _call_log_hash(), Insert new CRM_DairyAEList ('Daily Call Logs') rows, skipping ones already…, sync_daily_call_logs(), check_auto_schedule(), check_daily_call_logs_auto_schedule(), check_pipeline_auto_schedule(), check_pnl_auto_schedule(), check_tier_alert_email_schedule() (+28 more)

### Community 19 - "compilerOptions"
Cohesion: 0.07
Nodes (26): compilerOptions, allowArbitraryExtensions, allowImportingTsExtensions, baseUrl, erasableSyntaxOnly, ignoreDeprecations, jsx, lib (+18 more)

### Community 20 - "BM25"
Cohesion: 0.12
Nodes (19): BM25, detect_domain(), _load_csv(), Load CSV and return list of dicts, Core search function using BM25, Auto-detect the most relevant domain from query, Main search function with auto-domain detection, Search across all domains and combine results (+11 more)

### Community 21 - "crm_backfills.py"
Cohesion: 0.22
Nodes (24): activate_next_chunk(), cancel_backfill(), chunk_ranges(), create_backfill(), create_incremental(), earliest_state(), incremental_range(), pause_backfill() (+16 more)

### Community 22 - "Tailwind CSS Utility Reference"
Cohesion: 0.05
Nodes (43): Arbitrary Values, Aspect Ratio, Background Colors, Border Color, Border Radius, Border Style, Border Width, Borders (+35 more)

### Community 23 - "BM25"
Cohesion: 0.12
Nodes (19): BM25, detect_domain(), _load_csv(), Load CSV and return list of dicts, Core search function using BM25, Auto-detect the most relevant domain from query, Main search function with auto-domain detection, Search across all domains and combine results (+11 more)

### Community 24 - "models.py"
Cohesion: 0.08
Nodes (37): High-speed parallel CRM synchronization engine for Customer 360. Fetches and…, AccountExecutive, ActivityLog, AeImportBatch, AeImportRow, AeReassignmentLog, AeTarget, Base (+29 more)

### Community 25 - "spacing"
Cohesion: 0.09
Nodes (22): $type, $value, $type, $value, $type, $value, $type, $value (+14 more)

### Community 26 - "DesignSystemGenerator"
Cohesion: 0.13
Nodes (12): DesignSystemGenerator, Find matching reasoning rule for a category., Apply reasoning rules to search results., Select best matching result based on priority keywords., Extract results list from search result dict., Generate complete design system recommendation. variance/motion/density are…, Bucket a 1-10 dial value into its tier config. Returns None if value is None., Generates design system recommendations from aggregated searches. (+4 more)

### Community 27 - "RuntimeError"
Cohesion: 0.08
Nodes (9): downgrade(), downgrade(), downgrade(), downgrade(), downgrade(), downgrade(), downgrade(), downgrade() (+1 more)

### Community 28 - "components.json"
Cohesion: 0.09
Nodes (22): aliases, components, hooks, lib, ui, utils, iconLibrary, menuAccent (+14 more)

### Community 29 - "Customer360.tsx"
Cohesion: 0.07
Nodes (24): react, ActivityLog, CallLog, CompanyDetail, CompanyShipment, CalendarDayButton(), ContainerScroll(), ActivityTab() (+16 more)

### Community 30 - ".agents/skills/design-system/scripts/generate-slide.py"
Cohesion: 0.15
Nodes (19): _e(), generate_chart_slide(), generate_cta_slide(), generate_deck(), generate_metrics_slide(), generate_problem_slide(), generate_solution_slide(), generate_testimonial_slide() (+11 more)

### Community 31 - "api.ts"
Cohesion: 0.04
Nodes (53): AccountExecutive, adminApi, AdminUser, AdminUserPatch, AeImportBatch, AeImportRow, AEPerformanceResponse, AeReassignmentLogEntry (+45 more)

### Community 32 - "compilerOptions"
Cohesion: 0.10
Nodes (19): compilerOptions, allowImportingTsExtensions, erasableSyntaxOnly, lib, module, moduleDetection, noEmit, noFallthroughCasesInSwitch (+11 more)

### Community 33 - ".opencode/skills/design-system/scripts/generate-slide.py"
Cohesion: 0.15
Nodes (19): _e(), generate_chart_slide(), generate_cta_slide(), generate_deck(), generate_metrics_slide(), generate_problem_slide(), generate_solution_slide(), generate_testimonial_slide() (+11 more)

### Community 34 - "DesignSystemGenerator"
Cohesion: 0.12
Nodes (12): DesignSystemGenerator, Find matching reasoning rule for a category., Apply reasoning rules to search results., Select best matching result based on priority keywords., Extract results list from search result dict., Generate complete design system recommendation. variance/motion/density are…, Bucket a 1-10 dial value into its tier config. Returns None if value is None., Generates design system recommendations from aggregated searches. (+4 more)

### Community 35 - "color"
Cohesion: 0.11
Nodes (19): $type, $value, background, foreground, muted-foreground, primary, primary-hover, secondary (+11 more)

### Community 36 - "color"
Cohesion: 0.11
Nodes (19): $type, $value, background, foreground, muted-foreground, primary, primary-hover, secondary (+11 more)

### Community 37 - ".agents/skills/design-system/scripts/fetch-background.py"
Cohesion: 0.17
Nodes (17): generate_css_for_background(), get_background_image(), get_curated_images(), get_overlay_css(), get_pexels_search_url(), load_backgrounds_config(), load_brand_colors(), main() (+9 more)

### Community 38 - "TailwindConfigGenerator"
Cohesion: 0.11
Nodes (10): Generate Tailwind CSS configuration files., Add full color palette (50-950 shades) for a base color. Args: name: Color name…, TailwindConfigGenerator, Test adding custom fonts., Test adding custom spacing., Test plugin recommendations., Test initialization for JavaScript config., Test writing config to invalid path. (+2 more)

### Community 39 - ".opencode/skills/design-system/scripts/fetch-background.py"
Cohesion: 0.17
Nodes (17): generate_css_for_background(), get_background_image(), get_curated_images(), get_overlay_css(), get_pexels_search_url(), load_backgrounds_config(), load_brand_colors(), main() (+9 more)

### Community 40 - "TailwindConfigGenerator"
Cohesion: 0.11
Nodes (10): Generate Tailwind CSS configuration files., Add full color palette (50-950 shades) for a base color. Args: name: Color name…, TailwindConfigGenerator, Test adding custom fonts., Test adding custom spacing., Test plugin recommendations., Test validating config with empty theme extensions., Test initialization for JavaScript config. (+2 more)

### Community 41 - "BM25"
Cohesion: 0.15
Nodes (9): BM25, _normalize(), Apply synonym substitution before tokenizing., BM25 ranking algorithm for text search, Lowercase, normalize synonyms, split, remove punctuation, filter stopwords, Build BM25 index from documents, Score all documents against query, All indexed terms, for suggestion/typo-recovery purposes. (+1 more)

### Community 42 - "BusinessAnalytics.tsx"
Cohesion: 0.29
Nodes (3): BusinessAnalytics(), SEGMENTS, useDebounce()

### Community 43 - "BM25"
Cohesion: 0.10
Nodes (21): BM25, _domain_keywords(), _get_bm25(), _load_csv(), _load_product_keywords(), _normalize(), Apply synonym substitution before tokenizing., BM25 ranking algorithm for text search (+13 more)

### Community 44 - ".opencode/skills/ui-ux-pro-max/scripts/design_system.py"
Cohesion: 0.10
Nodes (25): ansi_ljust(), _detect_page_type(), format_ascii_box(), format_markdown(), format_master_md(), format_page_override_md(), generate_design_system(), _generate_intelligent_overrides() (+17 more)

### Community 45 - ".agents/skills/design/scripts/icon/generate.py"
Cohesion: 0.20
Nodes (15): apply_color(), apply_viewbox_size(), extract_svgs(), generate_batch(), generate_icon(), generate_sizes(), load_env(), main() (+7 more)

### Community 46 - "fontSize"
Cohesion: 0.12
Nodes (16): $type, $value, $type, $value, $type, $value, $type, $value (+8 more)

### Community 47 - "ShadcnInstaller"
Cohesion: 0.23
Nodes (9): main(), Add all available shadcn/ui components. Args: overwrite: If True, overwrite…, Handle shadcn/ui component installation., List installed components. Returns: Tuple of (success, message with component…, Check if shadcn is initialized in project. Returns: True if components.json…, Get list of already installed components. Returns: List of installed component…, Read shadcn version from project package.json; fall back to a pinned default., Add shadcn/ui components. Args: components: List of component names to add… (+1 more)

### Community 48 - "main"
Cohesion: 0.12
Nodes (8): main(), Add custom font families. Args: fonts: Dict of font_type: [font_names] e.g.,…, Add custom spacing values. Args: spacing: Dict of name: value e.g., {'18':…, Add custom breakpoints. Args: breakpoints: Dict of name: width e.g., {'3xl':…, Add plugin requirements. Args: plugins: List of plugin names e.g.,…, Get plugin recommendations based on configuration. Returns: List of recommended…, Validate configuration. Returns: Tuple of (valid, message), Add custom colors to theme. Args: colors: Dict of color_name: color_value Value…

### Community 49 - "utils.py"
Cohesion: 0.23
Nodes (14): import_manifest(), match_company(), preview_manifest(), Session, read_file(), clean(), decimal(), extract_icris_from_tracking() (+6 more)

### Community 50 - ".opencode/skills/design/scripts/icon/generate.py"
Cohesion: 0.20
Nodes (15): apply_color(), apply_viewbox_size(), extract_svgs(), generate_batch(), generate_icon(), generate_sizes(), load_env(), main() (+7 more)

### Community 51 - "fontSize"
Cohesion: 0.12
Nodes (16): $type, $value, $type, $value, $type, $value, $type, $value (+8 more)

### Community 52 - "ShadcnInstaller"
Cohesion: 0.23
Nodes (9): main(), Add all available shadcn/ui components. Args: overwrite: If True, overwrite…, Handle shadcn/ui component installation., List installed components. Returns: Tuple of (success, message with component…, Check if shadcn is initialized in project. Returns: True if components.json…, Get list of already installed components. Returns: List of installed component…, Read shadcn version from project package.json; fall back to a pinned default., Add shadcn/ui components. Args: components: List of component names to add… (+1 more)

### Community 53 - "main"
Cohesion: 0.12
Nodes (8): main(), Add custom font families. Args: fonts: Dict of font_type: [font_names] e.g.,…, Add custom spacing values. Args: spacing: Dict of name: value e.g., {'18':…, Add custom breakpoints. Args: breakpoints: Dict of name: width e.g., {'3xl':…, Add plugin requirements. Args: plugins: List of plugin names e.g.,…, Get plugin recommendations based on configuration. Returns: List of recommended…, Validate configuration. Returns: Tuple of (valid, message), Add custom colors to theme. Args: colors: Dict of color_name: color_value Value…

### Community 54 - "TestShadcnInstaller"
Cohesion: 0.12
Nodes (9): Test adding components that are already installed., Test adding components in dry run mode., Test ShadcnInstaller class., Test adding all components without config., Test listing installed components without config., Test listing installed components when none exist., Test initialization with default project root., Test getting installed components when files exist. (+1 more)

### Community 55 - "radius"
Cohesion: 0.19
Nodes (14): $type, $value, $type, $value, $type, $value, primitive, radius (+6 more)

### Community 56 - "TestShadcnInstaller"
Cohesion: 0.12
Nodes (9): Test adding components that are already installed., Test adding components in dry run mode., Test ShadcnInstaller class., Test adding all components without config., Test listing installed components without config., Test listing installed components when none exist., Test initialization with default project root., Test getting installed components when files exist. (+1 more)

### Community 57 - ".agents/skills/ui-ux-pro-max/scripts/design_system.py"
Cohesion: 0.18
Nodes (14): ansi_ljust(), _detect_page_type(), format_ascii_box(), format_page_override_md(), _generate_intelligent_overrides(), hex_to_ansi(), Format a page-specific override file with intelligent AI-generated content., Generate intelligent overrides based on page type using layered search. Uses… (+6 more)

### Community 58 - ".agents/skills/brand/scripts/extract-colors.cjs"
Cohesion: 0.22
Nodes (11): calculateCompliance(), colorDistance(), displayPalette(), extractHexColors(), findNearestBrandColor(), fs, generateImageMagickCommand(), hexToRgb() (+3 more)

### Community 59 - ".agents/skills/brand/scripts/validate-asset.cjs"
Cohesion: 0.25
Nodes (13): checkManifest(), formatBytes(), formatOutput(), fs, main(), parseFilename(), path, RULES (+5 more)

### Community 60 - "pie-context.tsx"
Cohesion: 0.07
Nodes (36): ChartStatFlow(), ChartStatFlowFormat, ChartStatFlowProps, defaultChartStatFlowFormat, formatStatValue(), useNumberFlowElementReady(), PieCenter(), PieCenterProps (+28 more)

### Community 61 - "chart-tooltip.tsx"
Cohesion: 0.09
Nodes (38): ChartConfigContext, ChartConfigProviderProps, ChartConfigValue, DEFAULT_CHART_CONFIG, resolveTooltipBoxMotion(), SpringConfig, useChartConfig(), chartCssVars (+30 more)

### Community 62 - ".opencode/skills/brand/scripts/extract-colors.cjs"
Cohesion: 0.22
Nodes (11): calculateCompliance(), colorDistance(), displayPalette(), extractHexColors(), findNearestBrandColor(), fs, generateImageMagickCommand(), hexToRgb() (+3 more)

### Community 63 - ".opencode/skills/brand/scripts/validate-asset.cjs"
Cohesion: 0.25
Nodes (13): checkManifest(), formatBytes(), formatOutput(), fs, main(), parseFilename(), path, RULES (+5 more)

### Community 64 - "test_pipeline_sync.py"
Cohesion: 0.21
Nodes (21): _detect_pushed_followups(), Flags deals that were overdue and unresolved in the previous snapshot but…, Run-tracked wrapper around sync_active_pipeline — the single path both the…, Replace the pipeline_items table with the current CRM Active Pipeline snapshot.…, run_active_pipeline_sync(), sync_active_pipeline(), pipeline_html(), Builds valid Active Pipeline grid HTML with n data rows, same shape as… (+13 more)

### Community 65 - "test_crm_parser.py"
Cohesion: 0.21
Nodes (14): Regression test for the live page's actual structure: the header lives alone in…, read(), test_detail_header_totals_rows_and_exact_headers(), test_empty_detail_is_visible_warning(), test_invalid_date_rejected(), test_invalid_number_warns_without_zero(), test_login_and_changed_structure_detected(), test_manifest_list_and_explicit_dates() (+6 more)

### Community 66 - ".agents/skills/design-system/templates/design-tokens-starter.json"
Cohesion: 0.15
Nodes (12): component, $type, $value, dark, semantic, $schema, $type, $value (+4 more)

### Community 67 - ".opencode/skills/design-system/templates/design-tokens-starter.json"
Cohesion: 0.15
Nodes (12): component, $type, $value, dark, semantic, $schema, $type, $value (+4 more)

### Community 68 - ".agents/skills/design-system/scripts/validate-tokens.cjs"
Cohesion: 0.24
Nodes (11): extensions, formatReport(), fs, getFiles(), main(), parseArgs(), path, patterns (+3 more)

### Community 69 - "card"
Cohesion: 0.20
Nodes (12): $type, $value, bg, bg, padding, shadow, card, bg (+4 more)

### Community 70 - ".generate_config_string"
Cohesion: 0.20
Nodes (6): Generate configuration file content. Returns: Configuration file as string, Generate TypeScript configuration., Generate JavaScript configuration., Format plugins array for config. Validates each plugin name against a strict…, Add indentation to JSON string., Write configuration to file. Returns: Tuple of (success, message)

### Community 71 - ".opencode/skills/design-system/scripts/validate-tokens.cjs"
Cohesion: 0.24
Nodes (11): extensions, formatReport(), fs, getFiles(), main(), parseArgs(), path, patterns (+3 more)

### Community 72 - "card"
Cohesion: 0.20
Nodes (12): $type, $value, bg, bg, padding, shadow, card, bg (+4 more)

### Community 73 - ".generate_config_string"
Cohesion: 0.20
Nodes (6): Generate configuration file content. Returns: Configuration file as string, Generate TypeScript configuration., Generate JavaScript configuration., Format plugins array for config. Validates each plugin name against a strict…, Add indentation to JSON string., Write configuration to file. Returns: Tuple of (success, message)

### Community 74 - ".agents/skills/brand/scripts/inject-brand-context.cjs"
Cohesion: 0.31
Nodes (10): extractColorsFromTable(), extractCoreAttributes(), extractHexColors(), extractImageStyle(), extractTypography(), extractVoice(), fs, generatePromptAddition() (+2 more)

### Community 75 - ".agents/skills/design-system/scripts/embed-tokens.cjs"
Cohesion: 0.18
Nodes (8): args, fs, minimal, MINIMAL_TOKENS, path, projectRoot, tokensPath, wrapStyle

### Community 76 - "patch"
Cohesion: 0.18
Nodes (6): patch, Test adding components with overwrite flag., Test successful component addition., Test component addition with subprocess error., Test component addition when npx is not found., Test successful addition of all components.

### Community 77 - ".agents/skills/ui-styling/scripts/tests/test_tailwind_config_gen.py"
Cohesion: 0.22
Nodes (8): parametrize, Tests for tailwind_config_gen.py, Reduce a generated TS/JS config to a bare assignable object so it can be handed…, Regression guard for the missing-comma bug between the ``theme`` block and…, The property preceding ``plugins`` must end with a comma (pure-Python check, so…, The emitted config parses as valid JS via ``node --check``., _strip_to_object(), TestGeneratedConfigIsValidJs

### Community 78 - "search"
Cohesion: 0.24
Nodes (6): Main search function with auto-domain detection, search(), format_output(), Format results for Claude consumption (token-optimized), Known query -> expected top-domain sanity checks (not exact-row pinning, since…, TestSearchDomains

### Community 79 - ".opencode/skills/brand/scripts/inject-brand-context.cjs"
Cohesion: 0.31
Nodes (10): extractColorsFromTable(), extractCoreAttributes(), extractHexColors(), extractImageStyle(), extractTypography(), extractVoice(), fs, generatePromptAddition() (+2 more)

### Community 80 - ".opencode/skills/design-system/scripts/embed-tokens.cjs"
Cohesion: 0.18
Nodes (8): args, fs, minimal, MINIMAL_TOKENS, path, projectRoot, tokensPath, wrapStyle

### Community 81 - "duration"
Cohesion: 0.20
Nodes (10): fast, normal, slow, $type, $value, $type, $value, duration (+2 more)

### Community 82 - "patch"
Cohesion: 0.18
Nodes (6): patch, Test adding components with overwrite flag., Test successful component addition., Test component addition with subprocess error., Test component addition when npx is not found., Test successful addition of all components.

### Community 83 - ".opencode/skills/ui-styling/scripts/tests/test_tailwind_config_gen.py"
Cohesion: 0.22
Nodes (8): parametrize, Tests for tailwind_config_gen.py, Reduce a generated TS/JS config to a bare assignable object so it can be handed…, Regression guard for the missing-comma bug between the ``theme`` block and…, The property preceding ``plugins`` must end with a comma (pure-Python check, so…, The emitted config parses as valid JS via ``node --check``., _strip_to_object(), TestGeneratedConfigIsValidJs

### Community 84 - "search"
Cohesion: 0.24
Nodes (6): Main search function with auto-domain detection, search(), format_output(), Format results for Claude consumption (token-optimized), Known query -> expected top-domain sanity checks (not exact-row pinning, since…, TestSearchDomains

### Community 85 - ".agents/skills/design/scripts/logo/generate.py"
Cohesion: 0.29
Nodes (9): enhance_prompt(), generate_batch(), generate_logo(), load_env(), main(), Enhance the logo prompt with style and industry modifiers, Generate a logo using Gemini models with image generation Args: aspect_ratio:…, Generate multiple logo variants with different styles (+1 more)

### Community 86 - ".agents/skills/design-system/scripts/generate-tokens.cjs"
Cohesion: 0.36
Nodes (9): flattenTokens(), fs, generateCSS(), generateTailwind(), main(), parseArgs(), path, resolveReference() (+1 more)

### Community 87 - "use-chart-interaction.ts"
Cohesion: 0.18
Nodes (13): TooltipData, computeSegmentBounds(), INACTIVE_SEGMENT, SegmentBounds, ChartInteractionResult, ScaleLinear, ScaleTime, useChartInteraction() (+5 more)

### Community 88 - "duration"
Cohesion: 0.20
Nodes (10): fast, normal, slow, $type, $value, $type, $value, duration (+2 more)

### Community 89 - "._base_config"
Cohesion: 0.22
Nodes (6): Any, Path, Initialize generator. Args: typescript: If True, generate .ts config, else .js…, Determine default output path., Create base configuration structure., Get default content paths for framework.

### Community 90 - "Changes shipped this session"
Cohesion: 0.13
Nodes (14): 1. ICRIS extraction from UPS tracking numbers, 2. Data quality issue resolution for 1Z shipments, 3. CrmSync — merged "Trigger Sync" and "Sync Active Pipeline" into one button, 4. Email system — tier-alert digests activated, 5. Weekly report feature (new), 6. Notification clarity — bell alert descriptions, 7. Active Pipeline sync — converged robust path, 8. Data Quality — merge-target picker and bulk provisional merge (+6 more)

### Community 92 - "dropdown-menu.tsx"
Cohesion: 0.20
Nodes (8): DropdownMenuCheckboxItem, DropdownMenuContent, DropdownMenuItem, DropdownMenuLabel, DropdownMenuRadioItem, DropdownMenuSeparator, DropdownMenuSubContent, DropdownMenuSubTrigger

### Community 93 - ".opencode/skills/design/scripts/logo/generate.py"
Cohesion: 0.29
Nodes (9): enhance_prompt(), generate_batch(), generate_logo(), load_env(), main(), Enhance the logo prompt with style and industry modifiers, Generate a logo using Gemini models with image generation Args: aspect_ratio:…, Generate multiple logo variants with different styles (+1 more)

### Community 94 - ".opencode/skills/design-system/scripts/generate-tokens.cjs"
Cohesion: 0.36
Nodes (9): flattenTokens(), fs, generateCSS(), generateTailwind(), main(), parseArgs(), path, resolveReference() (+1 more)

### Community 95 - "button"
Cohesion: 0.20
Nodes (10): fg, font-size, hover-bg, button, $type, $value, $type, $value (+2 more)

### Community 96 - "._base_config"
Cohesion: 0.22
Nodes (6): Any, Path, Initialize generator. Args: typescript: If True, generate .ts config, else .js…, Determine default output path., Create base configuration structure., Get default content paths for framework.

### Community 97 - ".agents/skills/brand/scripts/sync-brand-to-tokens.cjs"
Cohesion: 0.33
Nodes (8): adjustBrightness(), { execFileSync }, extractColorsFromMarkdown(), fs, generateColorScale(), main(), path, updateDesignTokens()

### Community 98 - "_run"
Cohesion: 0.28
Nodes (8): CompletedProcess, Path, Regression tests for validate-tokens.cjs. The validator used to skip any line…, A hardcoded hex on the same line as a var() token is still a violation., A line that references only tokens produces no false positives., _run(), test_flags_hardcoded_hex_sharing_line_with_token(), test_token_only_line_reports_no_violation()

### Community 99 - "dependencies"
Cohesion: 0.07
Nodes (29): axios, clsx, echarts, dependencies, axios, clsx, echarts, geist (+21 more)

### Community 100 - "UniversalSearch.tsx"
Cohesion: 0.38
Nodes (6): SearchResultItem, FILTER_CHIPS, fmt$(), PRESET_RECENTS, UniversalSearch(), useDebounce()

### Community 101 - ".agents/skills/ui-ux-pro-max/scripts/core.py"
Cohesion: 0.21
Nodes (12): _domain_keywords(), _get_bm25(), _load_csv(), _load_product_keywords(), Load CSV and return list of dicts, with mtime-based caching., Fitted BM25 index for this file+columns, with mtime-based caching., Core search function using BM25. Returns (results, bm25_or_none)., Nearest known vocabulary terms for a query that returned 0 hits, so the caller… (+4 more)

### Community 102 - "sheet.tsx"
Cohesion: 0.22
Nodes (6): SheetContent, SheetContentProps, SheetDescription, SheetOverlay, SheetTitle, sheetVariants

### Community 103 - ".agents/skills/ui-ux-pro-max/scripts/tests/test_core.py"
Cohesion: 0.18
Nodes (11): format_markdown(), format_master_md(), generate_design_system(), persist_design_system(), Format design system as markdown., Main entry point for design system generation. Args: query: Search query (e.g.,…, Slugify a name into a single safe path segment. Only [a-z0-9_-] survives; every…, Persist design system to design-system/<project>/ folder using Master +… (+3 more)

### Community 104 - ".opencode/skills/brand/scripts/sync-brand-to-tokens.cjs"
Cohesion: 0.33
Nodes (8): adjustBrightness(), { execFileSync }, extractColorsFromMarkdown(), fs, generateColorScale(), main(), path, updateDesignTokens()

### Community 105 - "_run"
Cohesion: 0.28
Nodes (8): CompletedProcess, Path, Regression tests for validate-tokens.cjs. The validator used to skip any line…, A hardcoded hex on the same line as a var() token is still a violation., A line that references only tokens produces no false positives., _run(), test_flags_hardcoded_hex_sharing_line_with_token(), test_token_only_line_reports_no_violation()

### Community 106 - "remote-compose.sh"
Cohesion: 0.50
Nodes (8): compose(), die(), ensure_remote_dir(), init_remote(), remote(), remote-compose.sh script, sync_project(), usage()

### Community 107 - "button"
Cohesion: 0.20
Nodes (10): fg, font-size, hover-bg, button, $type, $value, $type, $value (+2 more)

### Community 108 - "navigation-menu.tsx"
Cohesion: 0.25
Nodes (7): NavigationMenu, NavigationMenuContent, NavigationMenuIndicator, NavigationMenuList, NavigationMenuTrigger, navigationMenuTriggerStyle, NavigationMenuViewport

### Community 109 - "select.tsx"
Cohesion: 0.25
Nodes (7): SelectContent, SelectItem, SelectLabel, SelectScrollDownButton, SelectScrollUpButton, SelectSeparator, SelectTrigger

### Community 110 - "input"
Cohesion: 0.29
Nodes (8): padding-x, input, $type, $value, focus-ring, padding-x, $type, $value

### Community 111 - "radius"
Cohesion: 0.19
Nodes (14): $type, $value, $type, $value, $type, $value, primitive, radius (+6 more)

### Community 112 - "detect_domain"
Cohesion: 0.43
Nodes (3): detect_domain(), Auto-detect the most relevant domain from query. Matches are weighted by…, TestDomainDetection

### Community 114 - "animation.ts"
Cohesion: 0.23
Nodes (6): clipRevealTransition(), DEFAULT_CHART_ENTER_TRANSITION, ChartRevealClip(), ChartRevealClipMode, ChartRevealClipProps, SpringOptions

### Community 115 - "main.py"
Cohesion: 0.06
Nodes (91): get_db(), admin_audit_logs(), ae_import_batches(), ae_import_rows(), ae_performance(), analytics_customers(), analytics_dashboard(), backfills() (+83 more)

### Community 116 - "MasterAirWaybills.tsx"
Cohesion: 0.16
Nodes (17): AirWaybills(), fmt$(), fmtDate(), fmtWeight(), ShipmentDetailDrawer(), shipmentWeight(), useDebounce(), AIRLINES (+9 more)

### Community 117 - "detect_domain"
Cohesion: 0.43
Nodes (3): detect_domain(), Auto-detect the most relevant domain from query. Matches are weighted by…, TestDomainDetection

### Community 118 - "Tailwind CSS Utility Reference"
Cohesion: 0.05
Nodes (43): Arbitrary Values, Aspect Ratio, Background Colors, Border Color, Border Radius, Border Style, Border Width, Borders (+35 more)

### Community 119 - "sm"
Cohesion: 0.60
Nodes (5): sm, sm, sm, $type, $value

### Community 120 - "Brand Guidelines v1.0"
Cohesion: 0.05
Nodes (37): 1. Color Palette, 2. Typography, 3. Logo Usage, 4. Voice & Tone, 5. Imagery Guidelines, 6. Design Components, Accessibility, AI Image Generation (+29 more)

### Community 121 - "area.tsx"
Cohesion: 0.08
Nodes (45): Area(), AreaProps, CurveFactory, AreaGradientDefs(), AreaGradientDefsProps, useAreaLoadingPulseState(), useChart(), useChartHover() (+37 more)

### Community 122 - "email_notifications.py"
Cohesion: 0.07
Nodes (48): Settings, _ae_codes_with_email(), _fmt_currency(), _format_admin_summary(), _format_ae_digest(), _format_weekly_admin(), _format_weekly_ae(), _html_wrap() (+40 more)

### Community 123 - "Brand Guidelines v1.0"
Cohesion: 0.05
Nodes (37): 1. Color Palette, 2. Typography, 3. Logo Usage, 4. Voice & Tone, 5. Imagery Guidelines, 6. Design Components, Accessibility, AI Image Generation (+29 more)

### Community 124 - "Design"
Cohesion: 0.06
Nodes (35): Banner Design (Built-in), Banner: Design Rules, Banner: Quick Size Reference, Banner: Top Art Styles, Banner: Workflow, CIP Design (Built-in), CIP: Generate Brief, CIP: Generate Mockups (+27 more)

### Community 125 - "radius"
Cohesion: 0.60
Nodes (5): radius, radius, radius, $type, $value

### Community 126 - "lg"
Cohesion: 0.60
Nodes (5): lg, $type, $value, lg, lg

### Community 127 - "Canvas Design System"
Cohesion: 0.06
Nodes (35): 1. Visual Communication First, 2. Minimal Text Integration, 3. Expert Craftsmanship, 4. Systematic Patterns, Analog Meditation, Approach, Canvas Boundaries, Canvas Design System (+27 more)

### Community 128 - "xl"
Cohesion: 0.67
Nodes (4): xl, xl, $type, $value

### Community 129 - "User"
Cohesion: 0.09
Nodes (47): customer_master_ready(), bulk_patch_quality(), cancel_crm_run(), crm_progress(), crm_rematch(), crm_run(), crm_run_payload(), crm_runs() (+39 more)

### Community 130 - "series-dash-tail-overlay.tsx"
Cohesion: 0.22
Nodes (9): DashTailStroke(), DashTailStrokeProps, EMPTY_METRICS, PathStrokeMetrics, resolveDashStartX(), resolveDashTailBounds(), SeriesDashTailOverlay, SeriesDashTailOverlayImpl() (+1 more)

### Community 131 - ".agents/skills/ui-ux-pro-max/scripts/validate_data.py"
Cohesion: 0.83
Nodes (3): _check_file(), main(), _read_rows()

### Community 132 - "key_insights"
Cohesion: 0.50
Nodes (4): key_insights(), _prune_notification_state(), Drops seen/snoozed entries for ids no longer live, so a resolved-then-recurring…, The notification-bell feed — high-severity alerts only (the full Alerts page…

### Community 133 - "normalize_icris"
Cohesion: 0.08
Nodes (54): AeImportBatch, analyze(), cell_text(), clean_header(), clean_text(), import_ae_assignments(), normalize_ae_value(), parse_workbook() (+46 more)

### Community 134 - "y-domain-utils.ts"
Cohesion: 0.14
Nodes (22): extractReferenceAreaConfigs(), getChildComponentName(), isReferenceAreaElement(), ReferenceAreaConfigProps, lerpDomain(), snapDomains(), tweenDomains(), useAnimatedYDomains() (+14 more)

### Community 135 - "xl"
Cohesion: 0.67
Nodes (4): xl, xl, $type, $value

### Community 136 - "none"
Cohesion: 0.67
Nodes (4): $type, $value, none, none

### Community 137 - ".opencode/skills/ui-ux-pro-max/scripts/validate_data.py"
Cohesion: 0.83
Nodes (3): _check_file(), main(), _read_rows()

### Community 138 - "devDependencies"
Cohesion: 0.50
Nodes (3): devDependencies, playwright, playwright

### Community 141 - "UUID"
Cohesion: 0.11
Nodes (54): add_alias(), admin_update_user(), ae_import_batch(), aliases(), archive_document(), assign_ae(), backfill_cancel(), backfill_detail() (+46 more)

### Community 142 - "Design"
Cohesion: 0.06
Nodes (35): Banner Design (Built-in), Banner: Design Rules, Banner: Quick Size Reference, Banner: Top Art Styles, Banner: Workflow, CIP Design (Built-in), CIP: Generate Brief, CIP: Generate Mockups (+27 more)

### Community 143 - "Canvas Design System"
Cohesion: 0.06
Nodes (35): 1. Visual Communication First, 2. Minimal Text Integration, 3. Expert Craftsmanship, 4. Systematic Patterns, Analog Meditation, Approach, Canvas Boundaries, Canvas Design System (+27 more)

### Community 144 - "auth.py"
Cohesion: 0.09
Nodes (30): upgrade(), AuthUserOut, create_session_token(), find_user_by_setup_token(), generate_setup_token(), get_ae_scope(), get_current_user(), hash_password() (+22 more)

### Community 145 - "Prerequisites"
Cohesion: 0.06
Nodes (33): Accessibility, Available Domains, Available Stacks, Common Rules for Professional UI, Common Sticking Points, Example Workflow, How to Use This Skill, Icons & Visual Elements (+25 more)

### Community 146 - "Prerequisites"
Cohesion: 0.06
Nodes (33): Accessibility, Available Domains, Available Stacks, Common Rules for Professional UI, Common Sticking Points, Example Workflow, How to Use This Skill, Icons & Visual Elements (+25 more)

### Community 147 - "Form & Input Components"
Cohesion: 0.06
Nodes (32): Accordion, Alert, Alert Dialog, Avatar, Badge, Button, Card, Checkbox (+24 more)

### Community 148 - "Tailwind CSS Responsive Design"
Cohesion: 0.06
Nodes (32): 1. Mobile-First Design, 2. Consistent Breakpoint Usage, 3. Test at Breakpoint Boundaries, 4. Use Container for Content Width, 5. Progressive Enhancement, 6. Avoid Too Many Breakpoints, Best Practices, Breakpoint System (+24 more)

### Community 149 - "x-axis.tsx"
Cohesion: 0.12
Nodes (21): hmsTimeFmt, shortDateFmt, weekdayDateFmt, allIndexLayouts(), AxisTick, binomial(), buildDataAlignedTicks(), composePositiveSum() (+13 more)

### Community 150 - "Form & Input Components"
Cohesion: 0.06
Nodes (32): Accordion, Alert, Alert Dialog, Avatar, Badge, Button, Card, Checkbox (+24 more)

### Community 154 - "Shipment"
Cohesion: 0.06
Nodes (74): ManifestDetail, number(), ParsedValue, _add_alias(), backfill_1z_icris_extraction(), _counter(), exact_company(), invalid_icris_reason() (+66 more)

### Community 158 - "DataQualityIssue"
Cohesion: 0.10
Nodes (17): DataQualityIssue, The 30-day buffer must cover crm_invalid_icris too, not just crm_blank_icris —…, crm_icris_not_in_master is a different situation entirely (waiting on a…, _seed_icris_issue(), test_buffer_applies_to_both_blank_and_invalid_icris(), test_icris_not_in_master_is_excluded_from_the_buffer(), test_manual_resolve_records_admin_as_resolved_by(), test_recent_icris_issue_is_pending_not_stuck() (+9 more)

### Community 159 - "Tailwind CSS Responsive Design"
Cohesion: 0.06
Nodes (32): 1. Mobile-First Design, 2. Consistent Breakpoint Usage, 3. Test at Breakpoint Boundaries, 4. Use Container for Content Width, 5. Progressive Enhancement, 6. Avoid Too Many Breakpoints, Best Practices, Breakpoint System (+24 more)

### Community 160 - "Typography Specifications"
Cohesion: 0.06
Nodes (30): Accessibility, Base System, Best Practices, Clean & Modern, Common Font Pairings, Contrast Requirements, CSS Implementation, Editorial (+22 more)

### Community 161 - "Typography Specifications"
Cohesion: 0.06
Nodes (30): Accessibility, Base System, Best Practices, Clean & Modern, Common Font Pairings, Contrast Requirements, CSS Implementation, Editorial (+22 more)

### Community 162 - "ring"
Cohesion: 0.67
Nodes (3): ring, $type, $value

### Community 163 - "Logo Usage Rules"
Cohesion: 0.07
Nodes (28): Absolute Don'ts, Approved Backgrounds, Before Using Logo, Clear Space, Co-branding, Color Rules, Color Usage, Color Variants (+20 more)

### Community 168 - "Component Specifications"
Cohesion: 0.07
Nodes (28): Alert, Anatomy, Anatomy, Anatomy, Anatomy, Anatomy, Badge, Button (+20 more)

### Community 169 - "shadcn/ui Accessibility Patterns"
Cohesion: 0.07
Nodes (28): Accordion, Alert, ARIA Labels, Checkbox and Radio, Color Contrast, Command Palette Navigation, Component-Specific Patterns, Dialog/Modal Navigation (+20 more)

### Community 174 - "Logo Usage Rules"
Cohesion: 0.07
Nodes (28): Absolute Don'ts, Approved Backgrounds, Before Using Logo, Clear Space, Co-branding, Color Rules, Color Usage, Color Variants (+20 more)

### Community 175 - "Component Specifications"
Cohesion: 0.07
Nodes (28): Alert, Anatomy, Anatomy, Anatomy, Anatomy, Anatomy, Badge, Button (+20 more)

### Community 176 - "shadcn/ui Accessibility Patterns"
Cohesion: 0.07
Nodes (28): Accordion, Alert, ARIA Labels, Checkbox and Radio, Color Contrast, Command Palette Navigation, Component-Specific Patterns, Dialog/Modal Navigation (+20 more)

### Community 180 - "Asset Approval Checklist"
Cohesion: 0.08
Nodes (25): Accessibility, Archival, Asset Approval Checklist, Automation Support, Color Compliance, Common Issues & Fixes, Content Accessibility, Content Quality (+17 more)

### Community 181 - "Logo AI Prompt Engineering"
Cohesion: 0.08
Nodes (25): Common Pitfalls, Core Prompt Structure, Detailed Brief, Eco/Sustainable, Effective Keywords by Style, Fashion Brand, Healthcare, Industry-Specific Prompts (+17 more)

### Community 184 - "Asset Approval Checklist"
Cohesion: 0.08
Nodes (25): Accessibility, Archival, Asset Approval Checklist, Automation Support, Color Compliance, Common Issues & Fixes, Content Accessibility, Content Quality (+17 more)

### Community 185 - "Logo AI Prompt Engineering"
Cohesion: 0.08
Nodes (25): Common Pitfalls, Core Prompt Structure, Detailed Brief, Eco/Sustainable, Effective Keywords by Style, Fashion Brand, Healthcare, Industry-Specific Prompts (+17 more)

### Community 186 - "Color Palette Management"
Cohesion: 0.08
Nodes (24): Accessibility Requirements, Brand Compliance Validation, Checking Contrast, Color Documentation Format, Color Extraction, Color Palette Examples, Color Palette Management, Color System Structure (+16 more)

### Community 189 - "2. Parser (`crm_parser.py`)"
Cohesion: 0.08
Nodes (25): 1. Connector (`crm_connector.py`), 2. Parser (`crm_parser.py`), 3. Sync engine (`crm_sync.py`), 4. Worker (`crm_worker.py`), 5. Backfills (`crm_backfills.py`), Auto-schedule, Claiming — the concurrency-safe part, CRM Sync (+17 more)

### Community 191 - "CIP Deliverable Guide"
Cohesion: 0.08
Nodes (24): Apparel, Business Card, Car/Sedan, CIP Deliverable Guide, Core Identity, Digital Assets, Email Signature, Envelope (+16 more)

### Community 192 - "App.tsx"
Cohesion: 0.09
Nodes (12): Settings(), CUSTOMER_TYPE_OPTIONS, EMPTY_FORM, FormState, STATUS_OPTIONS, ExecutiveOverview(), fmt$(), fmtNum() (+4 more)

### Community 193 - "States and Variants"
Cohesion: 0.08
Nodes (24): Accessibility, Accessibility Requirements, ARIA States, Color Contrast, Color Variants, Disabled States, Error Messages, Error States (+16 more)

### Community 201 - "DataQuality.tsx"
Cohesion: 0.14
Nodes (21): CompanyConflict, DataQualityIssue, CONFIDENCE_STYLES, conflictKey(), DataQuality(), DEFAULT_ISSUE_TYPES, FixDialog(), fmtDateTime() (+13 more)

### Community 203 - "UI Styling Skill"
Cohesion: 0.08
Nodes (24): Accessibility Patterns, Alternative: Tailwind-Only Setup, Best Practices, Common Patterns, Component Layer: shadcn/ui, Component Library Guide, Component + Styling Setup, Core Stack (+16 more)

### Community 204 - "Color Palette Management"
Cohesion: 0.08
Nodes (24): Accessibility Requirements, Brand Compliance Validation, Checking Contrast, Color Documentation Format, Color Extraction, Color Palette Examples, Color Palette Management, Color System Structure (+16 more)

### Community 207 - "Major"
Cohesion: 0.09
Nodes (23): ~~B-1~~ · Every API endpoint except `/auth/me` is unauthenticated — mostly fixed independently, remaining gap closed 2026-09-10, ~~B-2~~ · Session tokens are forgeable — ✅ FIXED 2026-09-10, ~~B-3 · `npm run build` fails — no bundle can be produced~~ — ✅ FIXED by 2026-09-09, B-4 · `npm run typecheck` does not exist, Blockers, Known Issues, M-10 · CRM parser silently stores corrupted MAWB destination/origin values, M-11 · Alerts page truncates at 100, unordered relative to relevance within severity (+15 more)

### Community 208 - "input"
Cohesion: 0.29
Nodes (8): padding-x, input, $type, $value, focus-ring, padding-x, $type, $value

### Community 209 - "test_customer_segments.py"
Cohesion: 0.17
Nodes (20): Session, Customer segmentation rule — decided 2026-09-03, full writeup in docs/customer-…, Recomputes `customer_type` for every company. Returns {tier: count}.…, recompute_customer_segments(), _cleanup(), _mk_company(), _mk_shipment(), A company with a huge spike 6 months ago and nothing since must still be Large… (+12 more)

### Community 210 - "API Reference"
Cohesion: 0.09
Nodes (22): Admin, Analytics, API Reference, Auth, Companies, Conventions, CRM sync, Data quality (+14 more)

### Community 211 - "grid.tsx"
Cohesion: 0.11
Nodes (23): ChartLoadingLabel(), ChartLoadingLabelProps, Grid(), GridProps, hideEdgeTicks(), resolveRowTickValues(), LINE_LOADING_PULSE_EASE, computeSeriesPathPoints() (+15 more)

### Community 213 - "Data Rules"
Cohesion: 0.10
Nodes (21): 10. CRM integration is read-only, 11. Deletion is archival, 12. Source spellings are preserved exactly, 13. Structure changes fail loudly, never silently, 14. Transactions and schema, 1. ICRIS is the customer identity, 2. Fuzzy matching is suggestion-only and is never auto-applied, 3. Manual links are never overwritten (+13 more)

### Community 215 - "CIP Deliverable Guide"
Cohesion: 0.08
Nodes (24): Apparel, Business Card, Car/Sedan, CIP Deliverable Guide, Core Identity, Digital Assets, Email Signature, Envelope (+16 more)

### Community 247 - "States and Variants"
Cohesion: 0.08
Nodes (24): Accessibility, Accessibility Requirements, ARIA States, Color Contrast, Color Variants, Disabled States, Error Messages, Error States (+16 more)

### Community 248 - "UI Styling Skill"
Cohesion: 0.08
Nodes (24): Accessibility Patterns, Alternative: Tailwind-Only Setup, Best Practices, Common Patterns, Component Layer: shadcn/ui, Component Library Guide, Component + Styling Setup, Core Stack (+16 more)

### Community 249 - "Workflow"
Cohesion: 0.08
Nodes (23): Art Direction Styles (Reuse from Banner), Color & Contrast, Design Best Practices, HTML Design Rules, HTML Template Structure, Option A: Chrome Headless CLI (Recommended — zero dependencies), Option B: chrome-devtools skill, Option C: Playwright script (+15 more)

### Community 250 - "Workflow"
Cohesion: 0.08
Nodes (23): Art Direction Styles (Reuse from Banner), Color & Contrast, Design Best Practices, HTML Design Rules, HTML Template Structure, Option A: Chrome Headless CLI (Recommended — zero dependencies), Option B: chrome-devtools skill, Option C: Playwright script (+15 more)

### Community 251 - "Design System"
Cohesion: 0.09
Nodes (22): Best Practices, Chart.js Integration, Command, Component Spec Pattern, Contextual Decision Flow, Decision System CSVs, Design System, Integration (+14 more)

### Community 252 - "Tailwind CSS Customization"
Cohesion: 0.09
Nodes (22): @apply Directive, Best Practices, Color Customization, Complete Tailwind Config, Configuration Examples, Content Configuration, Custom Color Palette, Custom Font Sizes (+14 more)

### Community 253 - "Design System"
Cohesion: 0.09
Nodes (22): Best Practices, Chart.js Integration, Command, Component Spec Pattern, Contextual Decision Flow, Decision System CSVs, Design System, Integration (+14 more)

### Community 254 - "Tailwind CSS Customization"
Cohesion: 0.09
Nodes (22): @apply Directive, Best Practices, Color Customization, Complete Tailwind Config, Configuration Examples, Content Configuration, Custom Color Palette, Custom Font Sizes (+14 more)

### Community 255 - "Routing by Task Type"
Cohesion: 0.10
Nodes (19): Banner Design Tasks, Brand Identity Tasks, Component Creation, Corporate Identity Program Tasks, Design Routing Guide, Design System Migration, Icon Design Tasks, Implementation Tasks (+11 more)

### Community 256 - "shadcn/ui Theming & Customization"
Cohesion: 0.10
Nodes (19): Base Color Presets, Best Practices, Color Customization, Color Format, Component Customization, CSS Variable System, Customize Styles, Customize Variants (+11 more)

### Community 257 - "Routing by Task Type"
Cohesion: 0.10
Nodes (19): Banner Design Tasks, Brand Identity Tasks, Component Creation, Corporate Identity Program Tasks, Design Routing Guide, Design System Migration, Icon Design Tasks, Implementation Tasks (+11 more)

### Community 258 - "shadcn/ui Theming & Customization"
Cohesion: 0.10
Nodes (19): Base Color Presets, Best Practices, Color Customization, Color Format, Component Customization, CSS Variable System, Customize Styles, Customize Variants (+11 more)

### Community 259 - "Asset Organization Guide"
Cohesion: 0.11
Nodes (18): Asset Entry (manifest.json), Asset Organization Guide, By Campaign, By Status, By Type, Cleanup Workflow, Components, Directory Structure (+10 more)

### Community 260 - "Primary Color Meanings"
Cohesion: 0.11
Nodes (18): Accessibility Considerations, Analogous, Black, Blue, Color Combinations by Industry, Color Harmony Types, Complementary, Green (+10 more)

### Community 261 - "Core Logo Types"
Cohesion: 0.11
Nodes (18): 1. Wordmark (Logotype), 2. Lettermark (Monogram), 3. Pictorial Mark (Brand Mark), 4. Abstract Mark, 5. Mascot, 6. Emblem, 7. Combination Mark, Aesthetic Styles (+10 more)

### Community 262 - "Customer 360 - AI Architecture & Domain Context"
Cohesion: 0.11
Nodes (18): 1. Executive Summary & Purpose, 2. Technology Stack & Infrastructure, 3. Data Models & Database Schema, 4. Non-Negotiable Domain Rules & Business Invariants, 5. Ingestion, Matching & Synchronization Pipelines, 6.1 Design Principles (Enterprise UI), 6.2 Application Routing & Views, 6. Frontend UI Architecture (+10 more)

### Community 263 - "Asset Organization Guide"
Cohesion: 0.11
Nodes (18): Asset Entry (manifest.json), Asset Organization Guide, By Campaign, By Status, By Type, Cleanup Workflow, Components, Directory Structure (+10 more)

### Community 264 - "Primary Color Meanings"
Cohesion: 0.11
Nodes (18): Accessibility Considerations, Analogous, Black, Blue, Color Combinations by Industry, Color Harmony Types, Complementary, Green (+10 more)

### Community 265 - "Core Logo Types"
Cohesion: 0.11
Nodes (18): 1. Wordmark (Logotype), 2. Lettermark (Monogram), 3. Pictorial Mark (Brand Mark), 4. Abstract Mark, 5. Mascot, 6. Emblem, 7. Combination Mark, Aesthetic Styles (+10 more)

### Community 266 - "Brand Consistency Checklist"
Cohesion: 0.11
Nodes (17): Audit Frequency, Brand Consistency Checklist, Channel Audit, Collateral, Colors, Common Issues, Email, Imagery (+9 more)

### Community 267 - "CIP Mockup Prompt Engineering"
Cohesion: 0.11
Nodes (17): Apparel (Polo/T-Shirt), Base Prompt Structure, Business Card, CIP Mockup Prompt Engineering, Context Modifiers, Corporate Minimal, Deliverable-Specific Modifiers, Letterhead (+9 more)

### Community 268 - "Color Semantics"
Cohesion: 0.11
Nodes (17): Accent, Applying Semantic Tokens, Background & Foreground, Border & Ring, Color Semantics, Dark Mode Overrides, Destructive, Interactive States (+9 more)

### Community 269 - "Brand Consistency Checklist"
Cohesion: 0.11
Nodes (17): Audit Frequency, Brand Consistency Checklist, Channel Audit, Collateral, Colors, Common Issues, Email, Imagery (+9 more)

### Community 270 - "CIP Mockup Prompt Engineering"
Cohesion: 0.11
Nodes (17): Apparel (Polo/T-Shirt), Base Prompt Structure, Business Card, CIP Mockup Prompt Engineering, Context Modifiers, Corporate Minimal, Deliverable-Specific Modifiers, Letterhead (+9 more)

### Community 271 - "Color Semantics"
Cohesion: 0.11
Nodes (17): Accent, Applying Semantic Tokens, Background & Foreground, Border & Ring, Color Semantics, Dark Mode Overrides, Destructive, Interactive States (+9 more)

### Community 272 - "Design System Master File"
Cohesion: 0.11
Nodes (17): Additional Forbidden Patterns, Anti-Patterns (Do NOT Use), Buttons, Cards, Color Palette, Component Specs, Design System Master File, Global Rules (+9 more)

### Community 273 - "Design System Master File"
Cohesion: 0.12
Nodes (16): Additional Forbidden Patterns, Anti-Patterns (Do NOT Use), Buttons, Cards, Color Palette, Component Specs, Design System Master File, Global Rules (+8 more)

### Community 274 - "Design Principles"
Cohesion: 0.12
Nodes (15): 22 Art Direction Styles, Banner Sizes & Art Direction Styles Reference, Complete Banner Sizes, CTA Rules, Design Principles, Pinterest Research Queries, Print, Print Specs (+7 more)

### Community 275 - "Design Principles"
Cohesion: 0.12
Nodes (15): 22 Art Direction Styles, Banner Sizes & Art Direction Styles Reference, Complete Banner Sizes, CTA Rules, Design Principles, Pinterest Research Queries, Print, Print Specs (+7 more)

### Community 276 - "Design Principles"
Cohesion: 0.12
Nodes (15): 22 Art Direction Styles, Banner Sizes & Art Direction Styles Reference, Complete Banner Sizes, CTA Rules, Design Principles, Pinterest Research Queries, Print, Print Specs (+7 more)

### Community 277 - "Design Principles"
Cohesion: 0.12
Nodes (15): 22 Art Direction Styles, Banner Sizes & Art Direction Styles Reference, Complete Banner Sizes, CTA Rules, Design Principles, Pinterest Research Queries, Print, Print Specs (+7 more)

### Community 278 - "CIP Design Reference"
Cohesion: 0.13
Nodes (14): CIP Brief (Start Here), CIP Design Reference, Commands, Deliverable Categories, Design Styles, Detailed References, Generate Mockups, HTML Presentation Features (+6 more)

### Community 279 - "Icon Design Reference"
Cohesion: 0.13
Nodes (14): Available Styles, CLI Options, Commands, Generate Batch Variations, Generate Multiple Sizes, Generate Single Icon, Icon Categories, Icon Design Reference (+6 more)

### Community 280 - "Copywriting Formulas"
Cohesion: 0.13
Nodes (14): AIDA (Attention-Interest-Desire-Action), Before-After-Bridge, Contrast Patterns, Copywriting Formulas, Core Formulas, Cost of Inaction, FAB (Features-Advantages-Benefits), Formula-to-Slide Mapping (+6 more)

### Community 281 - "Copywriting Formulas"
Cohesion: 0.13
Nodes (14): AIDA (Attention-Interest-Desire-Action), Before-After-Bridge, Contrast Patterns, Copywriting Formulas, Core Formulas, Cost of Inaction, FAB (Features-Advantages-Benefits), Formula-to-Slide Mapping (+6 more)

### Community 282 - "CIP Design Reference"
Cohesion: 0.13
Nodes (14): CIP Brief (Start Here), CIP Design Reference, Commands, Deliverable Categories, Design Styles, Detailed References, Generate Mockups, HTML Presentation Features (+6 more)

### Community 283 - "Icon Design Reference"
Cohesion: 0.13
Nodes (14): Available Styles, CLI Options, Commands, Generate Batch Variations, Generate Multiple Sizes, Generate Single Icon, Icon Categories, Icon Design Reference (+6 more)

### Community 284 - "Copywriting Formulas"
Cohesion: 0.13
Nodes (14): AIDA (Attention-Interest-Desire-Action), Before-After-Bridge, Contrast Patterns, Copywriting Formulas, Core Formulas, Cost of Inaction, FAB (Features-Advantages-Benefits), Formula-to-Slide Mapping (+6 more)

### Community 285 - "tasteskill: Anti-Slop Frontend Skill"
Cohesion: 0.18
Nodes (11): 0.A Read these signals first, 0.B Output a one-line "Design Read" before generating, 0. BRIEF INFERENCE (Read the Room Before Anything Else), 0.C If the brief is ambiguous, ask one question, do not guess, 0.D Anti-Default Discipline, 13. OUT OF SCOPE, 14. FINAL PRE-FLIGHT CHECK, 2.A When to reach for a real design system (use official packages) (+3 more)

### Community 286 - "Appendix B - Canonical Sources (read these before reinventing)"
Cohesion: 0.13
Nodes (15): Appendix B - Canonical Sources (read these before reinventing), Apple Liquid Glass (Apple platforms only), Atlassian, Bootstrap, Carbon, Fluent UI, GOV.UK, Material Web (+7 more)

### Community 287 - "Copywriting Formulas"
Cohesion: 0.13
Nodes (14): AIDA (Attention-Interest-Desire-Action), Before-After-Bridge, Contrast Patterns, Copywriting Formulas, Core Formulas, Cost of Inaction, FAB (Features-Advantages-Benefits), Formula-to-Slide Mapping (+6 more)

### Community 288 - "Banner Design - Multi-Format Creative Banner System"
Cohesion: 0.14
Nodes (13): Art Direction Styles (Top 10), Banner Design - Multi-Format Creative Banner System, Banner Size Quick Reference, Design Rules, Prerequisites, Security, Step 1: Gather Requirements (AskUserQuestion), Step 2: Research & Art Direction (+5 more)

### Community 289 - "Messaging Framework"
Cohesion: 0.14
Nodes (13): Core Statements, Elevator Pitches, Framework Structure, Message Architecture, Message by Audience, Message Testing, Messaging Framework, Mission Statement (+5 more)

### Community 290 - "Brand Voice Framework"
Cohesion: 0.14
Nodes (13): Brand Voice Framework, Character Spectrum, Emotion Spectrum, Language Spectrum, Step 1: Define Personality Traits, Step 2: Create Voice Chart, Step 3: Context Adaptation, Tone Spectrum (+5 more)

### Community 291 - "Layout Patterns"
Cohesion: 0.14
Nodes (13): Card Styles, Component Variants, CSS Structures, Feature Grid (3 columns), Layout Decision Flow, Layout Patterns, Layout Selection by Use Case, Metric Styles (+5 more)

### Community 292 - "Tailwind Integration"
Cohesion: 0.14
Nodes (13): Animation Tokens, Base Layer, Button Example, Component Classes, CSS Variables Setup, Dark Mode Toggle, HSL Format Benefits, shadcn/ui Alignment (+5 more)

### Community 293 - "Layout Patterns"
Cohesion: 0.14
Nodes (13): Card Styles, Component Variants, CSS Structures, Feature Grid (3 columns), Layout Decision Flow, Layout Patterns, Layout Selection by Use Case, Metric Styles (+5 more)

### Community 294 - "Banner Design - Multi-Format Creative Banner System"
Cohesion: 0.14
Nodes (13): Art Direction Styles (Top 10), Banner Design - Multi-Format Creative Banner System, Banner Size Quick Reference, Design Rules, Prerequisites, Security, Step 1: Gather Requirements (AskUserQuestion), Step 2: Research & Art Direction (+5 more)

### Community 295 - "Messaging Framework"
Cohesion: 0.14
Nodes (13): Core Statements, Elevator Pitches, Framework Structure, Message Architecture, Message by Audience, Message Testing, Messaging Framework, Mission Statement (+5 more)

### Community 296 - "Brand Voice Framework"
Cohesion: 0.14
Nodes (13): Brand Voice Framework, Character Spectrum, Emotion Spectrum, Language Spectrum, Step 1: Define Personality Traits, Step 2: Create Voice Chart, Step 3: Context Adaptation, Tone Spectrum (+5 more)

### Community 297 - "Layout Patterns"
Cohesion: 0.14
Nodes (13): Card Styles, Component Variants, CSS Structures, Feature Grid (3 columns), Layout Decision Flow, Layout Patterns, Layout Selection by Use Case, Metric Styles (+5 more)

### Community 298 - "Tailwind Integration"
Cohesion: 0.14
Nodes (13): Animation Tokens, Base Layer, Button Example, Component Classes, CSS Variables Setup, Dark Mode Toggle, HSL Format Benefits, shadcn/ui Alignment (+5 more)

### Community 299 - "Layout Patterns"
Cohesion: 0.14
Nodes (13): Card Styles, Component Variants, CSS Structures, Feature Grid (3 columns), Layout Decision Flow, Layout Patterns, Layout Selection by Use Case, Metric Styles (+5 more)

### Community 300 - ".agents/skills/brand/references/update.md"
Cohesion: 0.15
Nodes (12): Color Presets, Examples, Files Modified, Important, Overview, Skills Used, Step 1: Gather Brand Input, Step 2: Update Brand Guidelines (+4 more)

### Community 301 - "Logo Design Reference"
Cohesion: 0.15
Nodes (12): Available Styles, Color Psychology, Commands, Design Brief (Start Here), Detailed References, Generate Logo, Industry Defaults, Logo Design Reference (+4 more)

### Community 302 - "Token Architecture"
Cohesion: 0.15
Nodes (12): Categories, Dark Mode, File Organization, Layer 1: Primitive Tokens, Layer 2: Semantic Tokens, Layer 3: Component Tokens, Layer Overview, Migration from Flat Tokens (+4 more)

### Community 303 - ".opencode/skills/brand/references/update.md"
Cohesion: 0.15
Nodes (12): Color Presets, Examples, Files Modified, Important, Overview, Skills Used, Step 1: Gather Brand Input, Step 2: Update Brand Guidelines (+4 more)

### Community 304 - "Logo Design Reference"
Cohesion: 0.15
Nodes (12): Available Styles, Color Psychology, Commands, Design Brief (Start Here), Detailed References, Generate Logo, Industry Defaults, Logo Design Reference (+4 more)

### Community 305 - "Token Architecture"
Cohesion: 0.15
Nodes (12): Categories, Dark Mode, File Organization, Layer 1: Primitive Tokens, Layer 2: Semantic Tokens, Layer 3: Component Tokens, Layer Overview, Migration from Flat Tokens (+4 more)

### Community 306 - "Primitive Tokens"
Cohesion: 0.17
Nodes (11): Border Radius, Color Scales, Gray Scale, Motion / Duration, Primary Colors (Blue), Primitive Tokens, Shadows, Spacing Scale (+3 more)

### Community 307 - "Primitive Tokens"
Cohesion: 0.17
Nodes (11): Border Radius, Color Scales, Gray Scale, Motion / Duration, Primary Colors (Blue), Primitive Tokens, Shadows, Spacing Scale (+3 more)

### Community 308 - "4. DESIGN ENGINEERING DIRECTIVES (Bias Correction)"
Cohesion: 0.17
Nodes (12): 4.10 Quotes & Testimonials, 4.11 Page Theme Lock (Light / Dark Mode Consistency), 4.1 Typography, 4.2 Color Calibration, 4.3 Layout Diversification, 4.4 Materiality, Shadows, Cards, 4.5 Interactive UI States, 4.6 Data & Form Patterns (+4 more)

### Community 309 - "Core Visual Elements"
Cohesion: 0.18
Nodes (10): Color Palette, Colors, Core Visual Elements, Logo, Logo, Quick Checks, Typography, Typography (+2 more)

### Community 310 - "CIP Design Style Guide"
Cohesion: 0.18
Nodes (10): Bold Dynamic, CIP Design Style Guide, Classic Traditional, Color Psychology, Corporate Minimal, Fresh Modern, Luxury Premium, Modern Tech (+2 more)

### Community 311 - "Core Visual Elements"
Cohesion: 0.18
Nodes (10): Color Palette, Colors, Core Visual Elements, Logo, Logo, Quick Checks, Typography, Typography (+2 more)

### Community 312 - "CIP Design Style Guide"
Cohesion: 0.18
Nodes (10): Bold Dynamic, CIP Design Style Guide, Classic Traditional, Color Psychology, Corporate Minimal, Fresh Modern, Luxury Premium, Modern Tech (+2 more)

### Community 313 - "Customer 360"
Cohesion: 0.18
Nodes (10): Architecture, Customer 360, Data quality and backups, Documents and analytics, Import rules, Local Docker (optional), MAWB and source precedence, Read-only legacy CRM connector (+2 more)

### Community 314 - "Customer 360 domain rules"
Cohesion: 0.20
Nodes (9): Backend standalone (if DATABASE_URL points at accessible Postgres), Customer 360 domain rules, Developer commands, Key conventions, Local Docker, Remote (primary), Stack and layout, Testing (+1 more)

### Community 315 - "Brand"
Cohesion: 0.20
Nodes (9): Brand, Brand Sync Workflow, Quick Start, References, Routing, Scripts, Subcommands, Templates (+1 more)

### Community 316 - "Slide Strategies"
Cohesion: 0.20
Nodes (9): Common Structures, Duarte Sparkline Pattern, Matching Strategy to Context, Product Demo (6 slides), Sales Pitch (9 slides), Search Commands, Slide Strategies, Strategy Selection (+1 more)

### Community 317 - "Component Tokens"
Cohesion: 0.20
Nodes (9): Alert Tokens, Badge Tokens, Button Tokens, Card Tokens, Component Tokens, Dialog/Modal Tokens, Input Tokens, Table Tokens (+1 more)

### Community 318 - "Slide Strategies"
Cohesion: 0.20
Nodes (9): Common Structures, Duarte Sparkline Pattern, Matching Strategy to Context, Product Demo (6 slides), Sales Pitch (9 slides), Search Commands, Slide Strategies, Strategy Selection (+1 more)

### Community 319 - "UploadFile"
Cohesion: 0.33
Nodes (10): ae_import_commit(), ae_import_preview(), ae_targets_import(), company_import(), company_mapping(), company_preview(), read_ae_target_workbook(), read_ae_workbook() (+2 more)

### Community 320 - "frontend/package.json"
Cohesion: 0.18
Nodes (10): name, private, scripts, build, dev, lint, preview, typecheck (+2 more)

### Community 321 - "Brand"
Cohesion: 0.20
Nodes (9): Brand, Brand Sync Workflow, Quick Start, References, Routing, Scripts, Subcommands, Templates (+1 more)

### Community 322 - "Slide Strategies"
Cohesion: 0.20
Nodes (9): Common Structures, Duarte Sparkline Pattern, Matching Strategy to Context, Product Demo (6 slides), Sales Pitch (9 slides), Search Commands, Slide Strategies, Strategy Selection (+1 more)

### Community 323 - "Component Tokens"
Cohesion: 0.20
Nodes (9): Alert Tokens, Badge Tokens, Button Tokens, Card Tokens, Component Tokens, Dialog/Modal Tokens, Input Tokens, Table Tokens (+1 more)

### Community 324 - "10. REFERENCE VOCABULARY (Pattern Names the Agent Should Know)"
Cohesion: 0.20
Nodes (10): 10. REFERENCE VOCABULARY (Pattern Names the Agent Should Know), Animation Library Choice, Cards & Containers, Galleries & Media, Hero Paradigms, Layout & Grids, Micro-Interactions & Effects, Navigation & Menus (+2 more)

### Community 325 - "Slide Strategies"
Cohesion: 0.20
Nodes (9): Common Structures, Duarte Sparkline Pattern, Matching Strategy to Context, Product Demo (6 slides), Sales Pitch (9 slides), Search Commands, Slide Strategies, Strategy Selection (+1 more)

### Community 326 - "plugins"
Cohesion: 0.22
Nodes (8): plugins, rules, react/only-export-components, react/rules-of-hooks, $schema, oxc, typescript, warn

### Community 327 - "9. AI TELLS (Forbidden Patterns)"
Cohesion: 0.25
Nodes (8): 9.A Visual & CSS, 9. AI TELLS (Forbidden Patterns), 9.B Typography, 9.C Layout & Spacing, 9.D Content & Data ("Jane Doe" Effect), 9.E External Resources & Components, 9.F Production-Test Tells (banned outright), 9.G EM-DASH BAN (the single most-violated Tell)

### Community 328 - "Slides Reference"
Cohesion: 0.29
Nodes (6): Key Features, Knowledge Base, Slides Reference, Usage, When to Use, Workflow

### Community 329 - "HTML Slide Template"
Cohesion: 0.29
Nodes (6): Animation Classes, Background Images, Base Structure, Chart.js Integration, CSS Variables Reference, HTML Slide Template

### Community 330 - "HTML Slide Template"
Cohesion: 0.29
Nodes (6): Animation Classes, Background Images, Base Structure, Chart.js Integration, CSS Variables Reference, HTML Slide Template

### Community 331 - "Customer 360 Backend API Reference"
Cohesion: 0.29
Nodes (6): 1. Analytics & Executive Dashboard, 2. Companies & Customers, 3. Shipments & Packages, 4. CRM Synchronization & Data Quality, 5. File Imports & Backups, Customer 360 Backend API Reference

### Community 333 - "Open Questions"
Cohesion: 0.10
Nodes (20): Data semantics, New as of 2026-08-07, Open Questions, Operational, Product intent, Q10 — Is the stale July 28 `dist/` actually what's in front of users? *(~60%)*, Q11 — Was `AUTH_SECRET` meant to be set on the remote host? *(~55%)*, Q12 — Do you want the 117 unused-import errors fixed, or the tsconfig relaxed? (+12 more)

### Community 334 - "Slides Reference"
Cohesion: 0.29
Nodes (6): Key Features, Knowledge Base, Slides Reference, Usage, When to Use, Workflow

### Community 335 - "HTML Slide Template"
Cohesion: 0.29
Nodes (6): Animation Classes, Background Images, Base Structure, Chart.js Integration, CSS Variables Reference, HTML Slide Template

### Community 336 - "APPENDICES - Real Source-Backed Reference Material"
Cohesion: 0.29
Nodes (6): APPENDICES - Real Source-Backed Reference Material, Appendix A - Install Commands per Design System, Appendix C - Apple Liquid Glass: Honest Web Approximation, Safer web approximation skeleton, What is NOT official, What is official

### Community 337 - "11. REDESIGN PROTOCOL"
Cohesion: 0.29
Nodes (7): 11.A Detect the Mode (first action), 11.B Audit Before Touching, 11.C Preservation Rules, 11.D Modernisation Levers (priority order), 11.E Decision Tree: Targeted Evolution vs Full Redesign, 11.F What Never Changes Silently, 11. REDESIGN PROTOCOL

### Community 338 - "3. DEFAULT ARCHITECTURE & CONVENTIONS"
Cohesion: 0.29
Nodes (7): 3.A Stack, 3.B State, 3.C Icons, 3.D Emoji Policy, 3. DEFAULT ARCHITECTURE & CONVENTIONS, 3.E Responsiveness & Layout Mechanics, 3.F Dependency Verification (mandatory)

### Community 339 - "6. PERFORMANCE & ACCESSIBILITY GUARDRAILS"
Cohesion: 0.29
Nodes (7): 6.A Hardware Acceleration, 6.B Reduced Motion (mandatory), 6.C Dark Mode (mandatory for any consumer-facing page), 6.D Core Web Vitals Targets, 6.E DOM Cost, 6.F Z-Index Restraint, 6. PERFORMANCE & ACCESSIBILITY GUARDRAILS

### Community 340 - "HTML Slide Template"
Cohesion: 0.29
Nodes (6): Animation Classes, Background Images, Base Structure, Chart.js Integration, CSS Variables Reference, HTML Slide Template

### Community 341 - "Slides"
Cohesion: 0.33
Nodes (5): References (Knowledge Base), Routing, Slides, Subcommands, When to Use

### Community 342 - "CLAUDE.md"
Cohesion: 0.10
Nodes (19): Architecture, Backend (`backend/app/`), Commands, ⚠️ Current state you must know, Data-rule probe — run before and after touching sync/import/matching, Docker services, Environment, Frontend (+11 more)

### Community 343 - "Slides"
Cohesion: 0.33
Nodes (5): References (Knowledge Base), Routing, Slides, Subcommands, When to Use

### Community 344 - "Brand Guidelines Template"
Cohesion: 0.40
Nodes (4): Brand Guidelines Template, Document Structure, Extractable Fields, Usage

### Community 345 - "Rankings.tsx"
Cohesion: 0.13
Nodes (14): xlsx, CustomerAnalytics(), fmt$(), fmtAxisDate(), fmtNum(), CustomerMetric, DEFAULT_RANK_STYLE, fmt$() (+6 more)

### Community 346 - "sidebar-component.tsx"
Cohesion: 0.11
Nodes (6): DetailSidebar(), getSidebarContent(), MenuItemT, MenuSectionT, SidebarContent, svgPaths

### Community 347 - "Customer 360 System Memory Bank & Architecture Specification"
Cohesion: 0.40
Nodes (4): 🚀 2026 Enterprise Technology Stack & Architectural Standards, Customer 360 System Memory Bank & Architecture Specification, 📦 Domain Rules & Verification Order, 📊 Latest Test & Quality Metrics

### Community 348 - "Brand Guidelines Template"
Cohesion: 0.40
Nodes (4): Brand Guidelines Template, Document Structure, Extractable Fields, Usage

### Community 349 - "$type"
Cohesion: 0.60
Nodes (5): $type, $value, border, border, border

### Community 350 - "12. THE BLOCK LIBRARY (Contract - Implementations Land Here Iteratively)"
Cohesion: 0.40
Nodes (5): 12.A File Location, 12.B Required Frontmatter, 12.C Required Body Sections, 12.D Block-Library Discipline, 12. THE BLOCK LIBRARY (Contract - Implementations Land Here Iteratively)

### Community 351 - "5. CONTEXT-AWARE PROACTIVITY"
Cohesion: 0.40
Nodes (5): 5.A Sticky-Stack - Canonical Skeleton, 5.B Horizontal-Pan - Canonical Skeleton, 5.C Scroll-Reveal Stagger - Canonical Skeleton (lighter alternative), 5. CONTEXT-AWARE PROACTIVITY, 5.D Forbidden Animation Patterns

### Community 352 - "8. DARK MODE PROTOCOL"
Cohesion: 0.40
Nodes (5): 8.A Token Strategy (pick one, stick to it), 8.B Do Not Prescribe Specific Colors Here, 8.C Default Mode, 8.D Test in Both Modes Before Finishing, 8. DARK MODE PROTOCOL

### Community 353 - "Courier Intelligence Dashboard Design Reference"
Cohesion: 0.40
Nodes (4): Color Palette, Courier Intelligence Dashboard Design Reference, Layout Structure, Overview

### Community 354 - "value_error"
Cohesion: 0.50
Nodes (4): Request, ValueError, value_error(), exception_handler

### Community 355 - "React + TypeScript + Vite"
Cohesion: 0.50
Nodes (3): Expanding the Oxlint configuration, React Compiler, React + TypeScript + Vite

### Community 356 - "test_ae_targets.py"
Cohesion: 0.25
Nodes (16): clean_header(), extract_ae_code(), import_ae_targets(), _num(), parse_month(), parse_workbook(), Session, Import for AE monthly targets ("AE_Targets_2026.xlsx", 'All_AE_Flat' sheet):… (+8 more)

### Community 357 - "AEPerformance.tsx"
Cohesion: 0.16
Nodes (15): AECustomer, AEPerformanceItem, AeTarget, AEPerformance(), attainmentTone(), DATE_PRESET_GROUPS, DateRangeControl(), fmt$() (+7 more)

### Community 358 - "projection-utils.ts"
Cohesion: 0.20
Nodes (16): buildAutoFutureValues(), buildProjectionPath(), BuildProjectionPathOptions, buildTargetPath(), computeProjectionAnchorTangentSlope(), intervalFromAdjacentRows(), intervalFromSeriesSpan(), linearRegressionSlope() (+8 more)

### Community 359 - "7. DIAL DEFINITIONS (Technical Reference)"
Cohesion: 0.50
Nodes (4): 7. DIAL DEFINITIONS (Technical Reference), DESIGN_VARIANCE (Level 1-10), MOTION_INTENSITY (Level 1-10), VISUAL_DENSITY (Level 1-10)

### Community 360 - "Role-Based Access Control — Plan & Status"
Cohesion: 0.11
Nodes (16): 1. Local dev environment, 2. New feature: CRM Daily Call Logs, 3. UI cleanup pass (many small fixes, all verified live), 4. Data Quality: ICRIS Number Mismatch messaging, 5. Role-Based Access Control (the big one), 6. Where to pick up, Session Handover — Customer 360, 1. Roles (+8 more)

### Community 361 - "UX System Flow — Customer 360"
Cohesion: 0.12
Nodes (16): 1. Entry flow, 2. Navigation shell (present on every `/app/*` page), 3. Full route map (as built today), 4. Drill-down pattern (Customer Directory → Customer 360), 5. Command palette (Ctrl+K) behavior, 6. Consistency scorecard, 7. Docs drift (for context, not a UX finding), 8. Summary — top 5 things to fix for smoother navigation (+8 more)

### Community 362 - "Development"
Cohesion: 0.12
Nodes (16): Commands that do NOT work, Conventions when editing, Core, Coverage shape, CRM connector, Data-rule probe, Database access, Development (+8 more)

### Community 363 - "3. Feature work this session (chronological)"
Cohesion: 0.12
Nodes (15): 1. Deployment pipeline (established this session), 2. Local dev environment note, 3. Feature work this session (chronological), 4. Known issues / things to watch, 5. Where to pick up, Alerts page simplified, CRM matching investigation → "Re-check CRM" feature, Customer 360 → Shipments tab sorted by date (+7 more)

### Community 364 - "2. Findings by area"
Cohesion: 0.12
Nodes (15): 0. Environment actually used, 1. Required command results, 2. Findings by area, 3. Key data rules — verdict table, 4. Test coverage gaps, 5. What I did not test, 6. Cleanup note, A. Auth — **BROKEN (blocker)** (+7 more)

### Community 365 - "Session Handover — Customer 360"
Cohesion: 0.13
Nodes (14): 1. What's actually deployed vs. not, 2. AWB / MAWB page work, 3. Password setup / bulk user creation (deployed), 4. Leaderboard feature (deployed), 5. Local dev environment fix, 6. The `customer360.shangrilatours.com.np` subdomain (blocked, not done), 7. Data Quality — the big one this session, 8. Where to pick up (+6 more)

### Community 366 - "Core entities"
Cohesion: 0.13
Nodes (15): `ActivityLog` — `activity_logs`, `Company` — `companies`, `CompanyAlias` — `company_aliases`, `CompanyDocument` — `company_documents`, Core entities, CRM sync entities, Data Model, Data quality (+7 more)

### Community 367 - "Leaderboard.tsx"
Cohesion: 0.32
Nodes (6): LeaderboardEntry, bylineFor(), Leaderboard(), Metric, METRIC_META, rankLabel()

### Community 368 - "app-shell.tsx"
Cohesion: 0.22
Nodes (14): activeGroupFor(), AppShell(), bucketLabel(), cleanInsightDescription(), daysAgo(), GROUP_ORDER, INSIGHT_CATEGORY_ICON, insightIcon() (+6 more)

### Community 369 - "dialog.tsx"
Cohesion: 0.29
Nodes (4): DialogContent, DialogDescription, DialogOverlay, DialogTitle

### Community 370 - "$type"
Cohesion: 0.60
Nodes (5): $type, $value, border, border, border

### Community 371 - "radius"
Cohesion: 0.60
Nodes (5): radius, radius, radius, $type, $value

### Community 372 - "8"
Cohesion: 0.67
Nodes (3): $type, $value, 8

### Community 373 - "destructive-foreground"
Cohesion: 0.67
Nodes (3): destructive-foreground, $type, $value

### Community 374 - "muted"
Cohesion: 0.67
Nodes (3): muted, $type, $value

### Community 375 - "primary-foreground"
Cohesion: 0.67
Nodes (3): primary-foreground, $type, $value

### Community 376 - "secondary-foreground"
Cohesion: 0.67
Nodes (3): secondary-foreground, $type, $value

### Community 389 - "Session Handover — Customer 360"
Cohesion: 0.13
Nodes (14): 1. What's actually deployed vs. not, 2. AWB / MAWB page work, 3. Password setup / bulk user creation (deployed), 4. Leaderboard feature (deployed), 5. Local dev environment fix, 6. The `customer360.shangrilatours.com.np` subdomain (blocked, not done), 7. Data Quality — the big one this session, 8. Where to pick up (+6 more)

### Community 390 - ".agents/skills/design/scripts/cip/generate.py"
Cohesion: 0.23
Nodes (13): build_cip_prompt(), check_logo_required(), generate_cip_set(), generate_with_nano_banana(), load_env(), load_logo_image(), main(), Generate image using Gemini Nano Banana (native image generation) Supports two… (+5 more)

### Community 1291 - "1. Company-master import — LIVE"
Cohesion: 0.14
Nodes (14): 1. Company-master import — LIVE, 2. Excel manifest import — DISABLED, 3. Document upload — related but separate, Column mapping, Commit semantics, File format, Imports, Live history [verified] (+6 more)

### Community 1292 - "AnalyticsFilterBar.tsx"
Cohesion: 0.10
Nodes (14): COMPARE_MODE_OPTIONS, DateRangeControl(), fmtShortDate(), isoDaysAgo(), localIso(), SEGMENTS, SelectOption, TIMEFRAME_GROUPS (+6 more)

### Community 1293 - "Session Handoff Pt.2 — 2026-08-10"
Cohesion: 0.14
Nodes (13): 10. AE Assignment — new feature, built from a real spreadsheet, 1. Universal Search click-through was broken (backend + frontend), 2. Air Waybills filters — Date Range was completely dead, 3. System-wide hover-effect cleanup, 4. Business Analytics filter cleanup (Geography, Operational KPIs), 5. Customer 360 — PDF export, Analytics tab, Documents tab, 6. Customer Directory — Weight column + flexible filters, 7. Master Air Waybills — MAWB drawer was showing wrong/broken data (+5 more)

### Community 1294 - "Session Handover — Customer 360"
Cohesion: 0.14
Nodes (13): 10. Customer/Business Analytics — real "Unassigned" filter bug found and fixed, 11. Known issues spotted but NOT fixed (flagged only), 12. Where to pick up, 1. What's actually deployed vs. not, 2. Document Type (DOC) Analytics page, 3. The "transit airport vs. real destination" bug — fixed in 4 places, 4. Customer 360 — Origin filter removed, Destination filter self-emptying bug fixed, 5. Geography — "Top Countries by Revenue" chart removed (+5 more)

### Community 1295 - "Session Handover — Customer 360"
Cohesion: 0.14
Nodes (13): 10. Test suite growth this session, 11. Known issues / things to watch, 12. Where to pick up, 1. What's actually deployed vs. not, 2. Two-tier RBAC: `super_admin` vs `admin` — the big structural change, 3. Leaderboard — real Wins bug + shared DateRangeControl, 4. Small UI cleanups (each was a separate explicit request), 5. Data Quality page — redesigned, plus a real default-scope bug found (+5 more)

### Community 1296 - "1. Local Docker Workflow (Windows/Mac)"
Cohesion: 0.15
Nodes (12): 1. Local Docker Workflow (Windows/Mac), 2. Remote Development Workflow (Primary), Accessing the Services, Forwarding Ports to your Laptop, Initial Setup, Initial Setup (One-time), Prerequisites, Running Customer 360 (+4 more)

### Community 1297 - "loading-sweep.tsx"
Cohesion: 0.27
Nodes (9): BarLoadingSkeleton(), BarLoadingSkeletonProps, CurveFactory, generateEasedGradientStops(), getSkeletonHeights(), getSkeletonSigns(), hashFract(), LineLoadingSweepProps (+1 more)

### Community 1298 - "Alerts.tsx"
Cohesion: 0.20
Nodes (11): Alert, Alerts(), CATEGORIES, getIcon(), metricLabel(), SEVERITY_BAR, SEVERITY_DOT, SEVERITY_LABEL (+3 more)

### Community 1299 - "admin_bulk_create_users"
Cohesion: 0.19
Nodes (14): admin_bulk_create_users(), admin_create_user(), admin_issue_setup_link(), admin_list_users(), _create_user_no_password(), issue_setup_link(), log_activity(), Generates a fresh one-time setup token for `user` and returns the full link an… (+6 more)

### Community 1300 - "test_document_type_analytics.py"
Cohesion: 0.20
Nodes (8): One shipment per bill_type case, each PP-billed 100 so revenue is unambiguous., Document + Letter -> doc, Non-Doc -> non_doc, blank/NULL -> unclassified.…, Revenue split must follow the same PP/FC/FD-only convention as every other…, Each destination row must carry its own doc/non_doc counts so 'which…, _seed_doc_type_shipments(), test_document_letter_bucket_into_doc_non_doc_stays_separate_blank_is_unclassified(), test_document_type_by_destination_split(), test_document_type_revenue_uses_shared_revenue_expression()

### Community 1301 - "Frontend"
Cohesion: 0.18
Nodes (11): api.ts, Build and tooling, Components, Console health [verified], Data fetching pattern, Entry points, Frontend, Page inventory [verified — all 21 routes rendered and checked] (+3 more)

### Community 1302 - "Database"
Cohesion: 0.20
Nodes (10): Accuracy [verified 2026-08-06], Backup and restore, Database, Direct access, Key constraints, Live row counts [verified 2026-08-06], Migrations, ⚠️ One schema drift (+2 more)

### Community 1303 - "AeAssignment.tsx"
Cohesion: 0.48
Nodes (6): AeImportPreview, fmtDateTime(), fmtNum(), ImportTab(), RosterTab(), StatCard()

### Community 1304 - "auth.tsx"
Cohesion: 0.22
Nodes (6): App(), AuthContext, AuthContextValue, AuthProvider(), AuthStatus, queryClient

### Community 1305 - "Session Handoff — 2026-08-07 to 2026-08-10"
Cohesion: 0.20
Nodes (9): 1. Revenue / Pay Term fixes (backend), 2. Business Analytics filter unification (frontend), 3. Customer Directory + Customer Analytics fixes, 4. AE revenue reconciliation — methodology, for next time, 5. Administration pages (new), 6. Air Waybills / Master Air Waybills fixes, Loose ends / things worth knowing for next session, Session Handoff — 2026-08-07 to 2026-08-10 (+1 more)

### Community 1306 - "BM25"
Cohesion: 0.28
Nodes (5): BM25, BM25 ranking algorithm for text search, Lowercase, split, remove punctuation, filter short words, Build BM25 index from documents, Score all documents against query

### Community 1307 - "Architecture"
Cohesion: 0.22
Nodes (9): Architecture, Authentication flow, Backend module map (`backend/app/`), Docker Compose services, How a typical request flows, Request paths, Stack, The CRM ingestion pipeline (+1 more)

### Community 1308 - "Customer Segmentation Rule"
Cohesion: 0.22
Nodes (9): Customer Segmentation Rule, Email digest (Gmail SMTP), No manual-override exemption — deliberately, The rule, The "Tier Shipping Gap" alert, Verification, What this replaces, Where it lives (+1 more)

### Community 1309 - "BLOCKERS"
Cohesion: 0.22
Nodes (9): B-1 — Every API endpoint except `/auth/me` is unauthenticated, B-2 — Session tokens are forgeable: shipped default `AUTH_SECRET` is live, B-3 — `npm run build` fails; no production bundle can be produced, B-4 — `npm run typecheck` does not exist, yet is the documented verification step, BLOCKERS, Customer 360 — Confirmed Defects, M-1 — CRM sync overwrites a manually-edited `shipment_date`, M-2 — Company merge: non-unique React key makes bulk selection operate on the wrong rows (+1 more)

### Community 1310 - "test_mawbs_pnl_sort.py"
Cohesion: 0.39
Nodes (6): _cleanup(), _mk_mawb(), Regression: the frontend used to only re-sort the current (date-ordered) page…, A MAWB with pnl_bill_amount=0 must not blow up the ORDER BY with a SQL…, test_mawbs_sort_by_margin_does_not_error_on_zero_bill_amount(), test_mawbs_sort_by_margin_orders_across_full_result_set_not_just_one_page()

### Community 1311 - "lg"
Cohesion: 0.60
Nodes (5): lg, $type, $value, lg, lg

### Community 1312 - "leaderboard-rankings.tsx"
Cohesion: 0.25
Nodes (6): crownColorMap, LeaderboardRankingItem, LeaderboardRankings, LeaderboardRankingsProps, LeaderboardRow, pageSizeOptions

### Community 1313 - "Profitability.tsx"
Cohesion: 0.46
Nodes (7): addDays(), daysBetween(), fmt$(), fmt$Compact(), fmtDate(), pctChange(), Profitability()

### Community 1314 - "sm"
Cohesion: 0.60
Nodes (5): sm, sm, sm, $type, $value

### Community 1315 - "AeTargets.tsx"
Cohesion: 0.47
Nodes (4): AeTargets(), EditState, fmt$(), MONTHS

### Community 1316 - "DocumentTypeAnalytics.tsx"
Cohesion: 0.60
Nodes (3): DocumentTypeAnalytics(), fmt$(), fmtNum()

### Community 1317 - "Customer 360 — Documentation"
Cohesion: 0.40
Nodes (5): Contents, Conventions used in these docs, Customer 360 — Documentation, Ground truth as of 2026-08-06 **[verified]**, The one-paragraph version

### Community 1318 - "AlertBanner.tsx"
Cohesion: 0.40
Nodes (3): AlertBannerProps, AlertType, typeConfig

### Community 1319 - "OperationalAnalytics.tsx"
Cohesion: 0.70
Nodes (4): fmt$(), fmtDate(), fmtNum(), OperationalAnalytics()

### Community 1321 - "get_alerts"
Cohesion: 0.50
Nodes (3): get_alerts(), get, Session

### Community 1322 - "Pipeline.tsx"
Cohesion: 0.67
Nodes (5): formatDate(), formatMoney(), hoursAgo(), Pipeline(), relativeSyncTime()

### Community 1323 - "padding-y"
Cohesion: 0.67
Nodes (4): padding-y, padding-y, $type, $value

### Community 1324 - "none"
Cohesion: 0.67
Nodes (4): $type, $value, none, none

### Community 1338 - "padding-y"
Cohesion: 0.67
Nodes (4): padding-y, padding-y, $type, $value

### Community 1370 - "1. THE THREE DIALS (Core Configuration)"
Cohesion: 0.50
Nodes (4): 1.A Dial Inference (design read → dial values), 1.B Use-Case Presets, 1.C How the Dials Drive Output, 1. THE THREE DIALS (Core Configuration)

### Community 1372 - "16"
Cohesion: 0.67
Nodes (3): $type, $value, 16

### Community 1373 - "1"
Cohesion: 0.67
Nodes (3): $type, $value, 1

### Community 1374 - "3"
Cohesion: 0.67
Nodes (3): $type, $value, 3

### Community 1375 - "8"
Cohesion: 0.67
Nodes (3): $type, $value, 8

### Community 1376 - "destructive"
Cohesion: 0.67
Nodes (3): destructive, $type, $value

### Community 1377 - "destructive-foreground"
Cohesion: 0.67
Nodes (3): destructive-foreground, $type, $value

### Community 1378 - "muted"
Cohesion: 0.67
Nodes (3): muted, $type, $value

### Community 1379 - "primary-foreground"
Cohesion: 0.67
Nodes (3): primary-foreground, $type, $value

### Community 1380 - "ring"
Cohesion: 0.67
Nodes (3): ring, $type, $value

### Community 1381 - "secondary-foreground"
Cohesion: 0.67
Nodes (3): secondary-foreground, $type, $value

### Community 1382 - "12"
Cohesion: 0.67
Nodes (3): $type, $value, 12

### Community 1383 - "4"
Cohesion: 0.67
Nodes (3): $type, $value, 4

### Community 1384 - "6"
Cohesion: 0.67
Nodes (3): $type, $value, 6

### Community 1385 - "destructive"
Cohesion: 0.67
Nodes (3): destructive, $type, $value

## Knowledge Gaps
- **2707 isolated node(s):** `fs`, `path`, `fs`, `path`, `fs` (+2702 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **101 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `react` connect `react` to `chart-context.tsx`, `time-series-chart-shell.tsx`, `series-dash-tail-overlay.tsx`, `y-domain-utils.ts`, `AnalyticsFilterBar.tsx`, `loading-sweep.tsx`, `Alerts.tsx`, `api`, `x-axis.tsx`, `AeAssignment.tsx`, `auth.tsx`, `Customer360.tsx`, `api.ts`, `leaderboard-rankings.tsx`, `Profitability.tsx`, `AeTargets.tsx`, `DocumentTypeAnalytics.tsx`, `OperationalAnalytics.tsx`, `BusinessAnalytics.tsx`, `Pipeline.tsx`, `pie-context.tsx`, `chart-tooltip.tsx`, `App.tsx`, `plugins`, `DataQuality.tsx`, `grid.tsx`, `use-chart-interaction.ts`, `Rankings.tsx`, `sidebar-component.tsx`, `command.tsx`, `dropdown-menu.tsx`, `UniversalSearch.tsx`, `AEPerformance.tsx`, `sheet.tsx`, `navigation-menu.tsx`, `select.tsx`, `Leaderboard.tsx`, `app-shell.tsx`, `dialog.tsx`, `animation.ts`, `MasterAirWaybills.tsx`, `area.tsx`?**
  _High betweenness centrality (0.016) - this node is a cross-community bridge._
- **Why does `dependencies` connect `dependencies` to `react-map-gl`, `@tanstack/react-query`, `Customer360.tsx`, `@visx/curve`, `@visx/gradient`, `@base-ui/react`, `date-fns`, `cmdk`, `echarts-for-react`, `@carbon/icons-react`, `frontend/package.json`, `framer-motion`, `html2canvas`, `jspdf`, `lucide-react`, `maplibre-gl`, `motion`, `@number-flow/react`, `@radix-ui/react-avatar`, `@radix-ui/react-checkbox`, `@radix-ui/react-collapsible`, `@radix-ui/react-dialog`, `@radix-ui/react-dropdown-menu`, `@radix-ui/react-hover-card`, `@radix-ui/react-popover`, `@radix-ui/react-scroll-area`, `@radix-ui/react-select`, `@radix-ui/react-separator`, `@radix-ui/react-slot`, `@tanstack/react-table`, `react-is`, `@visx/event`, `tailwind-merge`, `@visx/group`, `@visx/responsive`, `@visx/scale`, `@visx/shape`, `class-variance-authority`, `Rankings.tsx`, `zod`, `d3-array`, `d3-shape`?**
  _High betweenness centrality (0.006) - this node is a cross-community bridge._
- **Why does `react` connect `Customer360.tsx` to `App.tsx`, `BusinessAnalytics.tsx`, `dependencies`, `AEPerformance.tsx`?**
  _High betweenness centrality (0.004) - this node is a cross-community bridge._
- **Are the 2 inferred relationships involving `User` (e.g. with `AuthUserOut` and `LoginRequest`) actually correct?**
  _`User` has 2 INFERRED edges - model-reasoned connections that need verification._
- **What connects `fs`, `path`, `fs` to the rest of the system?**
  _2707 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `chart-context.tsx` be split into smaller, more focused modules?**
  _Cohesion score 0.09291521486643438 - nodes in this community are weakly interconnected._
- **Should `time-series-chart-shell.tsx` be split into smaller, more focused modules?**
  _Cohesion score 0.05786090005844535 - nodes in this community are weakly interconnected._